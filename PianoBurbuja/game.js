...
const STAGES={
  hearts:{
    ...
    travelStart:.72,travelEnd:.60,hitWindow:.20,missWindow:.16,
  ...
  stars:{
    ...
    travelStart:.61,travelEnd:.50,hitWindow:.17,missWindow:.13,
    density:3.15,color:"rgba(255,211,109,.88)"
  },
  crowns:{
    id:"crowns",name:"CORONAS",short:"♛",filled:"♛",empty:"♔",
    difficulty:"EXTREMO",className:"crowns",
    min1:.82,min2:.91,min3:.97,
    travelStart:.56,travelEnd:.46,hitWindow:.155,missWindow:.12,
    density:3.85,color:"rgba(116,218,255,.90)"
  }
};
...
function stageReward(stageId){
  return stageId==="hearts"?1:stageId==="stars"?2:3;
}

function songPoints(r){
  return (r.hearts>=3?1:0)+(r.stars>=3?2:0)+(r.crowns>=3?3:0);
}
...
function installV3Interface(){
  ...
  if(progressGuide){
    progressGuide.innerHTML="<strong>Máximo 6 puntos por canción</strong><span>♥♥♥ = +1 · ★★★ = +2 · ♛♛♛ = +3. Cada logro solo cuenta una vez.</span>";
  }

  const sectionCopy=$("#songsScreen .section-copy");
  if(sectionCopy){
    sectionCopy.textContent="Orden fijo de 19 canciones. Cada una tiene Corazones, Estrellas y Coronas; las Coronas son el modo extremo.";
  }

  const homeFoot=$(".home-foot");
  if(homeFoot){
    homeFoot.textContent="♥♥♥ = 1 punto · ★★★ = 2 puntos · ♛♛♛ = 3 puntos. Máximo 6 por canción.";
  }
    ...
    <div class="modal-card stage-modal-card">
      ...
      <div class="stage-choices">
        ...
        <button class="stage-choice stars" data-stage="stars" type="button">
          ...
          <span class="stage-choice-rating star-text" data-rating="stars">☆☆☆<small>requiere ♥♥♥ · +2 pts</small></span>
        </button>
        <button class="stage-choice crowns" data-stage="crowns" type="button">
          ...
          <span class="stage-choice-rating crown-text" data-rating="crowns">♔♔♔<small>requiere ★★★ · +3 pts</small></span>
        </button>
      </div>
...
function openStagePicker(orderIndex){
  ...
  ["hearts","stars","crowns"].forEach(stageId=>{
    ...
    if(r[stageId]>=3){
      small.textContent=`COMPLETADO · ${reward} ${reward===1?"pt":"pts"}`;
    }else if(!unlocked){
      small.textContent=stageId==="stars"?"requiere ♥♥♥ · +2 pts":"requiere ★★★ · +3 pts";
    }else{
      small.textContent=`+${reward} ${reward===1?"pt":"pts"} al 3/3`;
    }
  });
      ...
      b.innerHTML=`
        ...
        <span class="song-side">
          ${unlocked?`
            ...
            <small>${songPoints(r)}/6 pts</small>
          `:`
...
function preventLaneOverlap(notes,stageId){
  /*
    Evita dos rectángulos demasiado juntos en LA MISMA columna.
    Dos fichas pueden caer al mismo tiempo si están en columnas diferentes,
    porque visualmente no se enciman.
  */
  const minSameLaneGap=
    stageId==="hearts"?.19:
    stageId==="stars"?.20:
    .235;

  const lastByLane=[-99,-99,-99,-99];
  const out=[];

  for(const note of [...notes].sort((a,b)=>a.time-b.time||a.lane-b.lane)){
    const lane=Math.max(0,Math.min(3,Number(note.lane)||0));
    if(note.time-lastByLane[lane] < minSameLaneGap)continue;
    lastByLane[lane]=note.time;
    out.push({...note,lane});
  }
  return out;
}

function normalizeStageNotes(notes,duration,maxDensity,stageId){
  const maxNotes=Math.floor(duration*maxDensity);
  let sorted=notes
    .filter(n=>n.time>.48&&n.time<duration-.16)
    .sort((a,b)=>a.time-b.time||a.lane-b.lane);

  sorted=preventLaneOverlap(sorted,stageId);
...
function buildStageMap(base,duration,stageId,key){
  ...
  if(stageId==="hearts"){
    ...
    return normalizeStageNotes(notes,duration,STAGES.hearts.density,"hearts");
  }
  ...
  if(stageId==="stars"){
    ...
    return normalizeStageNotes(notes,duration,STAGES.stars.density,"stars");
  }
  ...
  base.forEach((n,i)=>{
    ...
    if(gap>.24 && n.time+.135<duration-.17){
      notes.push({
        time:n.time+.135,
    ...
    if(gap>.40 && n.time+.285<duration-.17){
      notes.push({
        time:n.time+.285,
  ...
  return normalizeStageNotes(notes,duration,STAGES.crowns.density,"crowns");
}
...
function stageTravel(noteTime){
  ...
  const floor=activeStage==="hearts"?.56:activeStage==="stars"?.47:.43;
...
async function showStageIntro(){
  ...
  top.textContent="PREPÁRATE";
  strong.textContent=
    activeStage==="hearts"?"♥ CORAZONES":
    activeStage==="stars"?"★ ESTRELLAS":
    "♛ CORONAS";
  small.textContent=
    activeStage==="hearts"?"RÁPIDO · toca directamente las fichas":
    activeStage==="stars"?"DIFÍCIL · más velocidad, menos margen":
    "EXTREMO · no parpadees";

  f.className=`phase-flash intro intro-${activeStage}`;
  await sleep(1250);

  f.classList.add("intro-leave");
  await sleep(260);
  f.className="phase-flash hidden";

  /* Un segundo limpio después de que desaparece la presentación. */
  $("#board").classList.add("ready-wait");
  await sleep(1000);
  $("#board").classList.remove("ready-wait");
}
...
async function beginGame(){
  ...
  /* La presentación ocurre con música y fichas completamente detenidas. */
  await showStageIntro();
...
function spawnTile(note){
  ...
  /* pointerdown es el camino normal; touchstart sirve como respaldo en iPhone/WebKit. */
...
function hitTile(tile,ev){
  ...
  const cx=Number.isFinite(ev.clientX)?ev.clientX:(touch?.clientX??0);
  const cy=Number.isFinite(ev.clientY)?ev.clientY:(touch?.clientY??0);
  burst(cx,cy,cfg.color);
  showJudge(label);
}
...
function awardStagePoints(){
  /*
    Cada dificultad usa un songId distinto.
    Firebase guarda el máximo de ese logro:
      archivo.mp3::hearts = 1 punto
      archivo.mp3::stars  = 2 puntos
      archivo.mp3::crowns = 3 puntos

    Por eso repetir una dificultad completada no vuelve a sumar.
  */
  const reward=stageReward(activeStage);
  emitBurbujaPoints({
    game:"PianoBurbuja",
    songId:`${activeSong.file}::${activeStage}`,
    songTitle:`${activeSong.title} · ${STAGES[activeStage].name}`,
    earnedThisRun:reward,
    previousBest:0,
    newBest:reward,
    delta:reward,
    maxPerSong:reward
...
function finishGame(){
  ...
  */
  ...
  const reward=stageReward(activeStage);
  $("#finishPoints").textContent=rating>=3?`${reward}/${reward}`:`0/${reward}`;
  ...
  if(rating<3){
    ...
    $("#pointDelta").textContent=
      newlyCompleted
        ?`+${reward} ${reward===1?"punto":"puntos"} Burbuja por nuevo logro`
