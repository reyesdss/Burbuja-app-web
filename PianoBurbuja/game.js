...
function phaseRating(h,m){const total=h+m;if(total<=0)return 0;const a=h/total;return a>=.95?3:a>=.86?2:a>=.72?1:0}
...
function loop(){if(!game?.running)return;const now=audio.currentTime,board=$('#board'),h=board.clientHeight,targetY=h-34-62-142+7;for(const t of game.tiles){if(t.hit||t.missed)continue;const p=(now-(t.time-t.travel))/t.travel,y=-146+p*(targetY+146);t.el.style.transform=`translateY(${y}px)`;if(now>t.time+.22)miss(t)}if(!phaseShown&&now>=phaseSplit){phaseShown=true;$('#phaseLabel').textContent='ESTRELLAS';$('#phaseLabel').className='phase-chip stars';$('#phaseHint').textContent='fase final';const f=$('#phaseFlash');f.classList.remove('hidden');setTimeout(()=>f.classList.add('hidden'),1250)}liveRatings();$('#phaseFill').style.width=Math.min(100,now/songDuration*100)+'%';$('#timeLeft').textContent=fmtTime(songDuration-now);if(now>=songDuration-.03||audio.ended){finishGame();return}raf=requestAnimationFrame(loop)}
function comboBonus(c){if(c>=120)return 4;if(c>=85)return 3;if(c>=55)return 2;if(c>=30)return 1;return 0}
function passedSong(hearts,stars,accuracy){
  return accuracy>=.72&&hearts>=1&&stars>=1;
}
function calculateBurbujaPoints(hearts,stars,accuracy,combo){
  if(!passedSong(hearts,stars,accuracy))return 0;
  let accuracyBonus=2;
  if(accuracy>=.98)accuracyBonus=10;
  else if(accuracy>=.95)accuracyBonus=8;
  else if(accuracy>=.90)accuracyBonus=6;
  else if(accuracy>=.82)accuracyBonus=4;
  const p=hearts+stars+accuracyBonus+comboBonus(combo);
...
function finishGame(){
  ...
  const hearts=phaseRating(game.firstHits,game.firstMisses);
  const stars=phaseRating(game.secondHits,game.secondMisses);
  const accuracy=accuracyAll();
  const passed=passedSong(hearts,stars,accuracy);
  const earned=calculateBurbujaPoints(hearts,stars,accuracy,game.bestCombo);
  const old=resultFor(activeSongIndex);
  const previous=old.bestPoints||0;
  const newBest=Math.max(previous,earned);
  const delta=Math.max(0,newBest-previous);

  save.results[String(activeSongIndex)]={
    hearts:Math.max(old.hearts||0,hearts),
    stars:Math.max(old.stars||0,stars),
    accuracy:Math.max(old.accuracy||0,accuracy),
    combo:Math.max(old.combo||0,game.bestCombo),
    score:Math.max(old.score||0,game.score),
    bestPoints:newBest
  };

  let unlockedName='';
  if(passed&&activeOrderIndex===save.unlocked-1&&save.unlocked<SONGS.length){
    save.unlocked++;
    unlockedName=songAtOrder(save.unlocked-1).title;
  }

  save.pianoPoints=totalBestPoints();
  persist();

  if(delta>0){
    emitBurbujaPoints({
      game:'PianoBurbuja',
      songId:activeSong.file,
  ...
  const final=resultFor(activeSongIndex);
  $('#finishTitle').textContent=passed?activeSong.title:`No superada · ${activeSong.title}`;
  $('#finishArtist').textContent=activeSong.artist;
  $('#finishHearts').textContent=symbols(final.hearts,'heart');
  $('#finishStars').textContent=symbols(final.stars,'star');
  $('#finishAccuracy').textContent=Math.round(accuracy*100)+'%';
  $('#finishCombo').textContent=game.bestCombo;
  $('#finishPoints').textContent=`${earned}/20`;

  if(!passed){
    $('#pointDelta').textContent='0 puntos · necesitas 72% de precisión, 1 corazón y 1 estrella';
    $('#unlockText').textContent='No desbloqueaste la siguiente canción. Inténtalo otra vez.';
  }else{
    $('#pointDelta').textContent=delta>0
      ?`+${delta} puntos nuevos para Burbuja`
      :(earned===previous?'Igualaste tu mejor puntuación':'Tu mejor puntuación sigue siendo mayor');
    $('#unlockText').textContent=unlockedName
      ?`Nueva canción desbloqueada: ${unlockedName}`
      :(save.unlocked===SONGS.length?'Ya desbloqueaste las 19 canciones.':'Superada. Puedes repetirla para mejorar tu resultado.');
  }
