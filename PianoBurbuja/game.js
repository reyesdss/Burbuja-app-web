...
const STORAGE_KEY="pianoburbuja_final_v1",MAX_LEVEL_SECONDS=120,MAX_POINTS_PER_SONG=20;
...
function loadState(){try{const s=JSON.parse(localStorage.getItem(STORAGE_KEY)||"null");if(s&&Array.isArray(s.order)&&s.order.length===SONGS.length)return s}catch(e){}const s={order:shuffle(SONGS.map((_,i)=>i)),unlocked:1,results:{},pianoPoints:0};localStorage.setItem(STORAGE_KEY,JSON.stringify(s));return s}
...
function emitBurbujaPoints(detail){try{window.dispatchEvent(new CustomEvent('burbuja:game-points',{detail}))}catch(e){}try{if(window.BurbujaGamePoints&&typeof window.BurbujaGamePoints.award==='function')window.BurbujaGamePoints.award(detail)}catch(e){}}
