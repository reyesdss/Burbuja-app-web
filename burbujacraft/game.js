...
function structureDescriptor(cx,cy){
 ...
 const kind=r<.34?"cabin":r<.60?"ruins":r<.82?"tower":"camp";
 const label=kind==="cabin"?"Cabaña abandonada":kind==="ruins"?"Ruinas del bosque":kind==="tower"?"Torre antigua":"Campamento abandonado";
...
function staticObjectAt(x,y){
 ...
 if(h<.075){const k=hash2(x+77,y+33);return{type:"tree",kind:k<.34?"oak":k<.67?"pine":"birch"}}
...
const BLOCKING_TYPES=new Set(["tree","rock","caveRock","ironNode","coalNode","volcanicNode","crimsonCrystalNode","ashRock","emberTree","woodWall","stoneWall","glassWall","abyssBlock","craftingTable","furnace","chest","bed","doorClosed","bubbleDoorClosed","tent"]);
...
function movePlayer(dx,dy,dt){
 let l=Math.hypot(dx,dy);if(!l)return;dx/=l;dy/=l;state.dirX=dx;state.dirY=dy;
 const terrain=terrainAt(state.x,state.y),swimming=terrain==="water",sp=(swimming?1.85:3.45)*dt,nx=state.x+dx*sp,ny=state.y+dy*sp;
...
function addItem(id,n=1,silent=false){
 if(!ITEM[id])return false;if(!state.inventory[id]&&inventoryDistinct()>=30){if(!silent)toast("Inventario lleno.");return false}
...
function useInventoryItem(id){
 ...
 else if(it.armor){state.armor=id;toast("Pechera equipada.",700)}
...
function breakInfo(o){
 ...
 const placedDrops={
   ...
   craftingTable:"craftingTable",furnace:"furnace",chest:"chest",bed:"bed",torch:"torch",marker:"marker"
 };
 if(placedDrops[o.type]||o.generated){
   const woodish=["woodWall","doorClosed","doorOpen","bubbleDoorClosed","bubbleDoorOpen","craftingTable","chest","bed","marker","tent","campfire"].includes(o.type);
   const need=woodish?"axe":"pick";
   const fallback=o.type==="stoneWall"?"stoneWall":o.type==="glassWall"?"glassWall":o.type==="abyssBlock"?"abyssBlock":o.type==="furnace"?"furnace":o.type==="chest"?"chest":o.type==="bed"?"bed":o.type==="marker"?"marker":o.type==="torch"?"torch":o.type.includes("Door")||o.type.startsWith("door")?"door":"woodWall";
...
function attack(){
 const now=performance.now();if(now-lastAttack<310)return;lastAttack=now;const it=selectedDef();
 ...
 if(no){
   ...
   if(resourceDamage[key]>=bi.hits){
     ...
     if(no.o.type==="tree")unlockAchievement("firstTree");
   }
   return;
 }
...
function useAction(){
 const now=performance.now();if(now-lastUse<240)return;lastUse=now;
...
function tryActivatePortal(){
 ...
 toast("❤️‍🔥 El portal al Abismo Carmesí está activo.",1500);tone(95,.35,.03);return true
}
...
function resolveFishing(){if(!fishing?.done)return;addItem("fish",1+((Math.random()*2)|0));fishing=null;unlockAchievement("fisher");tone(620,.06,.02)}
function enterCave(x,y){caveId=((Math.imul(x,73856093)^Math.imul(y,19349663)^worldSeed)>>>0);surfaceReturn={x:state.x,y:state.y};layer="cave";remoteMods={};const c=buildCave(caveId);state.x=c.cx+.5;state.y=c.cy+3.5;mobs=[];projectiles=[];unlockAchievement("cave");updateWorldLabel();toast("Entraste a una cueva.",900)}
...
function generatedLoot(x,y){
 ...
 if(s%3===0)loot.ironOre=1+(s%2);if(s%4===0)loot.rawBeef=1;if(s%5===0)loot.charcoal=2;
 return loot
}
...
function mobTemplate(type,x,y,id){
 const base={id:id||`${type}_${Date.now()}_${Math.random().toString(36).slice(2,7)}`,type,x,y,layer:layerKey(),dead:false,hit:0,attackCd:0,aggro:0,ai:Math.random()*10,homeX:x,homeY:y};
...
function spawnMobs(dt){
 ...
 let type=null;if(underground){if(c.hostiles>=6)return;const r=Math.random();type=r<.36?"zombie":r<.66?"spider":r<.88?"skeleton":"void"}
 else if(night){if(c.hostiles>=5)return;const r=Math.random();type=r<.38?"zombie":r<.66?"spider":r<.88?"skeleton":"void"}
...
function updateMobs(dt){
 ...
 for(const m of mobs){
   if(m.dead||m.layer!==layerKey())continue;m.hit=Math.max(0,m.hit-dt);m.attackCd=Math.max(0,m.attackCd-dt);m.aggro=Math.max(0,(m.aggro||0)-dt);m.ai+=dt;
...
function killMob(m,killer="local"){
 ...
 if(m.type==="zombie")drops.cloth=1;if(m.type==="spider")drops.string=1+((Math.random()*2)|0);if(m.type==="skeleton"){drops.bone=1;drops.arrow=2+((Math.random()*3)|0)}if(m.type==="void")drops.voidShard=1;
 if(killer==="remote"&&gameMode==="duo"&&online.isHost)online.sendAction({type:"grant",items:drops,hostileKill:!!m.hostile});
 else{for(const[id,n]of Object.entries(drops))addItem(id,n,true);if(m.hostile){hostileKills++;if(hostileKills>=10)unlockAchievement("hunter")}}
}
function handleRemoteAction(a){
 ...
 if(a.type==="grant"&&!online.isHost){for(const[id,n]of Object.entries(a.items||{}))addItem(id,n,true);if(a.hostileKill){hostileKills++;if(hostileKills>=10)unlockAchievement("hunter")}}
}
...
function damagePlayer(n){const armor=ITEM[state.armor]?.armor||0;n=Math.max(1,n*(1-armor));state.health=Math.max(0,state.health-n);tone(78,.08,.022);if(state.health<=0)respawnPlayer();updateHud()}
function respawnPlayer(){
 state.health=100;state.hunger=Math.max(35,state.hunger);layer="surface";caveId=null;remoteMods={};
 const sp=state.spawn||{x:SPAWN_X+.5,y:SPAWN_Y+.5};state.x=sp.x;state.y=sp.y;mobs=[];projectiles=[];
 toast(state.spawn?"Reapareciste en tu cama.":"Reapareciste en el origen.",900)
}
...
function updateDay(dt){
 ...
 const now=Math.floor(worldTime/480);days=now;if(now>prev&&now>=1)unlockAchievement("firstNight")
}
...
function updateContextHint(){
 let text="";const no=nearestObject(1.7);
 if(no){
   const t=no.o.type;
...
function updateWorldLabel(){
 if(layer==="cave")ui.world.textContent="CUEVAS PROFUNDAS";
 else if(layer==="abyss")ui.world.textContent="ABISMO CARMESÍ";
 else{const t=surfaceTerrainAt(state.x,state.y);ui.world.textContent=t==="water"?"AGUAS DE BURBUJA":t==="sand"?"COSTA DE BURBUJA":"MUNDO DE BURBUJA"}
}
function updatePartnerCard(){if(gameMode!=="duo"){ui.partnerCard.classList.remove("show");return}ui.partnerCard.classList.add("show");ui.partnerName.textContent=partnerProfile?.displayName||partnerProfile?.name||"Tu persona";ui.partnerDistance.textContent=remotePlayer&&remotePlayer.layer===layerKey()?Math.round(Math.hypot(remotePlayer.x-state.x,remotePlayer.y-state.y))+" m":"lejos"}
...
function saveObject(){return{version:4,worldSeed,worldTime,days,layer,caveId,surfaceReturn,mods,state,buildCount,hostileKills,savedAt:Date.now()}}
function saveGame(show=false){try{localStorage.setItem(gameMode==="duo"?"burbujacraft_v3_duo_local":"burbujacraft_v3_save",JSON.stringify(saveObject()))}catch{}if(show)toast("Partida guardada.",700);updateMenuStats()}
function loadGame(){try{return JSON.parse(localStorage.getItem("burbujacraft_v3_save"))}catch{return null}}
function applySave(s){
 ...
 buildCount=s.buildCount||0;hostileKills=s.hostileKills||0;remoteMods={};mobs=[];projectiles=[]
}
...
async function startDuo(){showModal("duoConnectingModal",false);$("duoConnectText").textContent="Comprobando tu sesión de Burbuja...";try{const res=await online.connectDuo(t=>$("duoConnectText").textContent=t);gameMode="duo";worldSeed=res.seed>>>0;partnerProfile=res.partner;worldTime=online.world?.worldTime||120;layer="surface";caveId=null;surfaceReturn=null;mods={};remoteMods={};mobs=[];projectiles=[];let local=null;try{local=JSON.parse(localStorage.getItem("burbujacraft_v3_duo_local"))}catch{}if(local?.worldSeed===worldSeed){applySave(local);gameMode="duo";worldSeed=res.seed>>>0}else{freshState(true);state.x=SPAWN_X+.5+(Math.random()-.5)*1.4;state.y=SPAWN_Y+.5+(Math.random()-.5)*1.4}online.onPartner((p,profile)=>{remotePlayer=p;partnerProfile=profile;updatePartnerCard()});online.onMods(applyRemoteChunk);online.onMobs((rm,isHost)=>{if(!isHost)mobs=rm.map(v=>({...v}))});online.onAction(handleRemoteAction);hideModal("duoConnectingModal",false);enterGame();flushPointEvents()}catch(e){hideModal("duoConnectingModal",false);const msg=e.message==="NO_AUTH"?"Inicia sesión primero en Burbuja desde el mismo dominio de GitHub Pages y vuelve a entrar al modo dúo.":e.message==="NO_PARTNER"?"No encontré a la segunda persona de Burbuja.":"No pude conectar el mundo dúo.";showMessage("MUNDO DÚO",msg)}}
...
function enterGame(){preferLandscape();setScreen("game");ui.mode.textContent=gameMode==="duo"?"DÚO":"SOLO";paused=false;running=true;saveTimer=0;onlineTimer=0;spawnTimer=0;dayRewardTimer=0;discoverTimer=0;activeChest=null;updateHotbar();updateHud();resize();cancelAnimationFrame(raf);last=performance.now();raf=requestAnimationFrame(loop)}
...
function loop(now){
 ...
 if(!paused){
   ...
   updateDay(dt);updateNeeds(dt);updateFishing();spawnMobs(dt);updateMobs(dt);updateProjectiles(dt);updateParticles(dt);updateOnline(dt);updateDiscovery(dt);dailyPlayReward(dt);duoReward(dt);
...
function draw(){
 ...
 drawTerrain();drawBuildPreview();drawObjects();drawProjectiles();drawMobs();drawPlayers();drawFishing();drawParticles();drawLighting()
}
function drawTerrain(){
 ...
 for(let y=b.y0;y<=b.y1;y++)for(let x=b.x0;x<=b.x1;x++){
  ...
  if(t==="grass"||t==="darkGrass"){
    ctx.fillStyle=t==="grass"?"#4f923f":"#427c39";ctx.fillRect(p.x,p.y,TILE+1,TILE+1);
    ctx.fillStyle=t==="grass"?"#61a84b":"#529343";ctx.fillRect(p.x+2,p.y+2,TILE-4,TILE-4);
...
function drawPlayerSprite(x,y,color,remote,dx,dy){
 ...
 ctx.fillStyle=dark;ctx.fillRect(-15,8,30,7);
 // brazos
...
$("attackBtn").addEventListener("pointerdown",e=>{e.preventDefault();attack()});$("useBtn").addEventListener("pointerdown",e=>{e.preventDefault();useAction()});$("placeBtn").addEventListener("pointerdown",e=>{e.preventDefault();placeAction()});$("soundBtn").onclick=()=>{soundOn=!soundOn;$("soundBtn").textContent=soundOn?"♪":"×"};
