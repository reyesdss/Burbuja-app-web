class BurbujacraftOnline {
  constructor(){
    this.ready=false;this.mode="offline";this.uid=null;this.partnerUid=null;this.partnerProfile=null;this.bubbleId=null;
  ...
  activeBubble(){
    ...
    const q=new URLSearchParams(location.search).get("bubbleId");
    this.bubbleId=q||localStorage.getItem("burbuja-active-bubble")||"";
  ...
  sharedDoc(id){const {doc}=this.mods;const b=this.activeBubble();if(!b)throw new Error("NO_BUBBLE");return doc(this.db,"bubbles",b,"shared",id)}
  ...
  async connectDuo(statusCb=()=>{}){
    ...
    const {doc,getDoc,setDoc,onSnapshot,serverTimestamp,updateDoc}=this.mods;
    ...
    if(!bubbleSnap.exists()||!(bubbleSnap.data().memberIds||[]).includes(this.uid))throw new Error("NOT_MEMBER");
    statusCb("Buscando a la otra persona de esta Burbuja...");
    const partnerUid=(bubbleSnap.data().memberIds||[]).find(id=>id!==this.uid);
    if(!partnerUid)throw new Error("NO_PARTNER");
    this.partnerUid=partnerUid;
    const partnerSnap=await getDoc(this.memberDoc(partnerUid));
    this.partnerProfile=partnerSnap.exists()?partnerSnap.data():{uid:partnerUid,displayName:"Tu persona"};
    const partner=this.partnerProfile;

    const worldRef=this.gameDoc("burbujacraft_world_v3");
    ...
    const partnerRef=this.gameDoc(`burbujacraft_v3_player_${this.partnerUid}`);
    ...
    const mobsRef=this.gameDoc("burbujacraft_v3_mobs");
    ...
    const actionRef=this.gameDoc(`burbujacraft_v3_action_${this.partnerUid}`);
  ...
  async publishState(state){
    ...
    const {doc,setDoc}=this.mods;try{await setDoc(this.gameDoc(`burbujacraft_v3_player_${this.uid}`),{...state,uid:this.uid,lastSeen:now},{merge:true})}catch{}
  }
  async publishMobs(mobs){
    ...
    const {doc,setDoc}=this.mods;try{await setDoc(this.gameDoc("burbujacraft_v3_mobs"),{hostUid:this.uid,updatedAt:now,mobs:mobs.slice(0,24)},{merge:true})}catch{}
  }
  async heartbeat(worldTime){
    ...
    const {doc,updateDoc}=this.mods;try{await updateDoc(this.gameDoc("burbujacraft_world_v3"),{hostHeartbeat:now,worldTime})}catch{}
  }
  async sendAction(action){
    if(this.mode!=="duo"||!this.uid)return;const {doc,setDoc}=this.mods;
    try{this.seq++;await setDoc(this.gameDoc(`burbujacraft_v3_action_${this.uid}`),{uid:this.uid,seq:this.seq,action,ts:Date.now()},{merge:true})}catch{}
  }
  chunkId(layer,x,y){
  ...
  watchChunks(layer,x,y){
    ...
    for(let dy=-1;dy<=1;dy++)for(let dx=-1;dx<=1;dx++){
      ...
      if(!this.chunkUnsubs.has(id)){
        const unsub=onSnapshot(this.gameDoc(id),s=>{if(s.exists()&&this.modsCallback)this.modsCallback(id,s.data().mods||{})});
  ...
  async syncMod(layer,x,y,value){
    ...
    const {doc,setDoc,updateDoc,FieldPath}=this.mods,{id}=this.chunkId(layer,x,y),key=`${Math.floor(x)}_${Math.floor(y)}`,ref=this.gameDoc(id);
    try{
  ...
  async awardPoints(eventId,amount,reason){
    try{
      ...
      const ref=this.sharedDoc("burbujacraftPoints"),uid=user.uid,claimKey=(uid+"_"+eventId).replace(/[^a-zA-Z0-9_-]/g,"_");
