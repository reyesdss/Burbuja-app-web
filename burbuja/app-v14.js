...
function switchView(view) {
  ...
  if (view === "Chat" && previous!=="Chat") {
    requestAnimationFrame(()=>requestAnimationFrame(()=>{scrollMessages();markRead()}));
  }
}
...
function wireRealtime() {
  ...
  state.unsubs.push(firebase.onSnapshot(bubbleCollection("members"), snap => {
    ...
    renderGames();
    if(state.partner)setTimeout(()=>repairStreakFromHistory(false),120);
  }));
...
function beginKeyboardSession(){
  const box=$("#messages");
  if(state.currentView!=="Chat"||!box)return;
  state.keyboardSession={wasBottom:nearBottom(120),scrollTop:box.scrollTop};
}
function endKeyboardSession(){state.keyboardSession=null}
function syncMobileViewport(resetBaseline=false){
  ...
  viewportRAF=requestAnimationFrame(()=>{
    ...
    if(resetBaseline||!editableFocused())viewportBaseline=Math.max(viewportBaseline,window.innerHeight,h);
    document.documentElement.style.setProperty("--app-height",`${h}px`);
    const focused=editableFocused();
    const keyboardOpen=focused&&(viewportBaseline-h)>110;
    document.documentElement.classList.toggle("keyboard-open",keyboardOpen);
    document.documentElement.classList.toggle("field-focused",focused);
    if(state.currentView==="Chat"&&state.keyboardSession){
      const box=$("#messages");
      if(box)requestAnimationFrame(()=>{
        if(state.keyboardSession?.wasBottom)scrollMessages();
        else if(state.keyboardSession)box.scrollTop=state.keyboardSession.scrollTop;
      });
    }
  });
}
...
function bindUI() {
  ...
  $("#messageInput").addEventListener("focus",()=>{
    ...
    setTimeout(()=>syncMobileViewport(false),20);
  });
  $("#messageInput").addEventListener("blur",()=>setTimeout(()=>{syncMobileViewport(true);endKeyboardSession()},120));
  ...
  window.addEventListener("orientationchange",()=>setTimeout(()=>syncMobileViewport(true),220));
