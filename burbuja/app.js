...
const BURBUJA_VERSION = "responsive-actions-refactor-20260919";
...
// Firebase Auth puede tardar unos instantes en restaurar la sesión.
// La interfaz preboot pinta la caché local antes de ese momento, por lo que
// las acciones que requieren identidad real deben esperar a que Auth confirme
// el usuario.
state.authReadyPromise = new Promise(resolve => { state.authReadyResolve = resolve; });

async function waitForAuthenticatedUser(timeoutMs = 5000) {
  const immediate = state.auth?.currentUser;
  if (immediate?.uid) {
    state.user = immediate;
    return immediate;
  }
  ...
  const retry = state.auth?.currentUser;
  if (retry?.uid) {
    state.user = retry;
    return retry;
  }
...
function authUid() {
  return state.auth?.currentUser?.uid || state.user?.uid || "";
}

function requireAuthUid() {
  const uid = authUid();
  if (!uid) throw new Error("La sesión todavía se está iniciando. Intenta de nuevo en un momento.");
  return uid;
}
...
function membershipCollection(uid = authUid()) {
  const safeUid = String(uid || "").trim();
  if (!safeUid) throw new Error("La sesión todavía se está iniciando. Intenta de nuevo en un momento.");
  return firebase.collection(state.db, "users", safeUid, "memberships");
}
function membershipDoc(bubbleId, uid = authUid()) {
  const safeUid = String(uid || "").trim();
  if (!safeUid) throw new Error("La sesión todavía se está iniciando. Intenta de nuevo en un momento.");
  return firebase.doc(state.db, "users", safeUid, "memberships", bubbleId);
}
...
async function loadMemberships() {
  const uid = authUid();
  if (!uid || !state.db || !firebase) return [];
  const snap = await firebase.getDocs(membershipCollection(uid));
  const resolved = await Promise.all(snap.docs.map(async d => {
    try {
      const b = await firebase.getDoc(firebase.doc(state.db,"bubbles",d.id));
      if (b.exists() && (b.data().memberIds || []).includes(uid)) return {id:d.id,...b.data()};
    } catch {}
    return null;
  }));
  ...
  state.bubbles = rows;
  try{localStorage.setItem(`burbuja-memberships-${uid}`,JSON.stringify(rows.map(b=>({id:b.id,name:b.name||"Nuestra Burbuja",emoji:b.emoji||"🫧",inviteCode:b.inviteCode||"",memberCount:b.memberCount||b.memberIds?.length||1,memberIds:b.memberIds||[],updatedAt:tsMillis(b.updatedAt)}))))}catch{}
  return rows;
}
...
async function showBubbleChooser(fromApp = false) {
  ...
  if (!user) {
    // Si la pantalla fue restaurada desde preboot pero no existe una sesión
    // real, no dejamos al usuario atrapado dentro de la app: lo llevamos
    // directamente al formulario de inicio de sesión.
    setPrebootMode(false);
    localStorage.setItem("burbuja-signed-out","1");
    state.user=null;
    state.activeBubbleId="";
    state.activeBubble=null;
    switchAuthTab("login");
    showScreen("auth");
    toast("Inicia sesión para administrar tus Burbujas.", 3200);
    return;
  }
...
async function createBubble(name, emoji) {
  const user = await waitForAuthenticatedUser();
  if (!user) throw new Error("Tu sesión todavía se está iniciando. Intenta de nuevo en un momento.");
...
function setPrebootMode(on){
  ...
  [
    // Estas dos acciones deben quedar disponibles durante el preboot:
    // permiten salir de una pantalla restaurada desde caché aunque Firebase
    // todavía no haya confirmado una sesión real.
    "saveProfileBtn","avatarPickerBtn","changePasswordBtn",
    "openStorageBtn","bubbleSwitcher","bubbleHubLogout"
  ].forEach(id=>{
    const el=$("#"+id);
...
function prebootFromLocalCache(){
  // Si el usuario pidió cerrar sesión, no volver a pintar el preboot
  // desde la caché local aunque todavía existan datos de una Burbuja.
  if(localStorage.getItem("burbuja-signed-out")==="1") return false;
...
async function loadFirebase() {
  ...
  firebase.onAuthStateChanged(state.auth, async (user) => {
    if (!state.authReady) {
      ...
      state.authReadyResolve?.(user || null);
    }
    ...
    try {
      // Una autenticación real vuelve a habilitar el preboot para futuras cargas.
      localStorage.removeItem("burbuja-signed-out");
...
async function saveDailyAnswer(){
  ...
  state.questionData={
    ...
    answers:{...previous.answers,[uid]:answer}
  };
...
function bindUI() {
  ...
  $("#bubbleHubLogout").addEventListener("click",async()=>{
    const user=await waitForAuthenticatedUser();
    try{if(state.activeBubbleId&&user)await updatePresence(false)}catch{}
    localStorage.setItem("burbuja-signed-out","1");
    localStorage.removeItem("burbuja-last-auth-uid");
    if(state.auth?.currentUser){try{await firebase.signOut(state.auth)}catch(e){console.warn("No se pudo cerrar la sesión de Firebase",e)}}
    clearSubscriptions();
    state.user=state.me=state.partner=state.accountProfile=null;
    state.activeBubbleId="";state.activeBubble=null;state.bubbles=[];
    setPrebootMode(false);
  ...
  $("#changePasswordBtn").addEventListener("click", async () => {
    ...
    try {
      await firebase.updatePassword(user, pw);
  ...
  $("#logoutBtn").addEventListener("click", async () => {
    // Este botón es también la salida de emergencia del preboot.
    // No esperamos a Auth: si Firebase todavía no restauró una sesión,
    // igualmente debemos sacar al usuario de la interfaz restaurada.
    ...
    // La presencia y el signOut ya no bloquean la navegación hacia Login.
    if (user && bubbleId) {
      try { await updatePresence(false); } catch {}
    }
    if (user) {
      try { await firebase.signOut(state.auth); }
      catch (err) { console.warn("No se pudo cerrar la sesión de Firebase", err); }
    }
    await updateSlotHint();
  });
...
// v15.2: pinta inmediatamente la última Burbuja desde localStorage.
// Firebase y Authentication continúan iniciando por detrás.
try{prebootFromLocalCache()}catch(e){console.warn("Preboot local no disponible",e)}
