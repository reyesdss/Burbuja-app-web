...
const BURBUJA_VERSION = "piano-launch-fix-20260905";
...
function switchView(view) {
  ...
  if(view==="Chat"&&previous!=="Chat"){
    requestAnimationFrame(()=>{if(!state.chatPositionRestored)restoreChatPositionOnce();void markRead().catch(()=>{})});
  }
  if(view==="Us"){
    renderBurbujaSummary();
    renderSpecialDates();
    void refreshMemoryCard(false).catch(()=>{});
  }
...
async function showBubbleChooser(fromApp = false) {
  ...
  showScreen("bubbles");
  closeBubbleForms();
  // La pantalla se abre de inmediato; la red solo actualiza los datos después.
  void loadMemberships().then(()=>renderBubbleChooser()).catch(e=>console.warn("No se pudo refrescar Mis Burbujas", e));
}
