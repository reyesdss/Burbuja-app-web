...
window.__burbujaDiag = window.__burbujaDiag || {};
window.__burbujaDiag.appLoaded = true;

const FIREBASE_VERSION = "12.18.0";
const BURBUJA_VERSION = "session-recovery-20260920";
...
function ensureLatestStylesheet(){
  try{
    const link=[...document.querySelectorAll('link[rel="stylesheet"]')]
      .find(l=>String(l.getAttribute("href")||"").includes("styles.css"));
    if(!link)return;
    const wanted="./styles.css?b=20260903-notas-fechas";
    if(!String(link.getAttribute("href")||"").includes("20260903-notas-fechas")){
...
const state = {
  ...
  pendingMessagePatches: new Map(),
  pendingStatuses: new Map(),
  pendingActivities: new Map(),
  pendingGameWrites: new Map(),
  pendingMemberPatch: {},
  pendingDailyQuestion: null,
  pendingPinned: null,
  pendingRelationship: null,
  pendingSpecialDates: null,
  pendingSavedMessages: null,
  pendingChatAppearance: null,
  pendingNoteData: null,
  dailyActiveWriteAt: 0,
  renderedMessageIds: [],
  renderedMessageClientIds: [],
  renderedMessageFirstUnreadId: "",
  renderedMessageSearch: "",
  messagesDirty: false
};
...
function showScreen(name) {
  // Limpiar bloqueos visuales inyectados en el HTML
  ["loadingScreen", "setupScreen", "authScreen", "bubbleScreen", "appShell"].forEach(id => {
    const el = document.getElementById(id);
    if (el) el.removeAttribute("style");
  });

  // Aplicar visibilidad por clases normales
...
// UI scheduler: Firestore can emit several snapshots in the same moment.
// Coalescing their renders into one animation frame keeps clicks and taps responsive.
const uiJobs = new Map();
let uiFrame = 0;
function scheduleUI(key, fn){
  if(typeof fn!=="function")return;
  uiJobs.set(key,fn);
  if(uiFrame)return;
  uiFrame=requestAnimationFrame(()=>{
    uiFrame=0;
    const jobs=[...uiJobs.values()];
    uiJobs.clear();
    for(const job of jobs){try{job()}catch(e){console.warn("UI render",e)}}
  });
}
function scheduleBackground(fn,timeout=1200){
  if(typeof fn!=="function")return;
  const run=()=>{try{Promise.resolve(fn()).catch(e=>console.warn("background task",e))}catch(e){console.warn("background task",e)}};
  if("requestIdleCallback" in window)requestIdleCallback(run,{timeout});
  else setTimeout(run,Math.min(timeout,700));
}
function cloneShallow(obj){return obj&&typeof obj==="object"?{...obj}:{};}
...
function forceLoginRequested(){
  return !!state.forceLogin || localStorage.getItem("burbuja-force-login")==="1";
}

function setForceLogin(on){
  state.forceLogin=!!on;
  try{
    if(on) localStorage.setItem("burbuja-force-login","1");
    else localStorage.removeItem("burbuja-force-login");
  }catch{}
}

function updateAuthActionButtons(){
  // El usuario ordenó dejar ambos botones visibles siempre en el perfil.
}

function clearVisualSession(){
  setPrebootMode(false);
  clearSubscriptions();
  state.user=null;
  state.me=null;
  state.partner=null;
...
function showLoginScreen(){
  ...
  state.authMode="forcing-login";
  setForceLogin(true);
  clearVisualSession();
  showScreen("auth");
  switchAuthTab("login");
  if (firebase && state.auth && state.auth.currentUser) {
    firebase.signOut(state.auth).catch(()=>{});
  }
}

window.burbujaOpenLogin = showLoginScreen;
...
async function handleLogout(){
  ...
  try {
    ...
    console.warn("Fallo al cerrar sesión en Firebase", e);
  }
  
  clearVisualSession();
  
  // Transición inmediata en vivo, SIN recargar la página
  showScreen("auth");
  switchAuthTab("login");
}
...
function switchView(view) {
  ...
  if(view==="Chat"&&previous!=="Chat"){
    if(state.messagesDirty)renderMessages();
    if(!state.chatPositionRestored)restoreChatPositionOnce();
    void markRead().catch(()=>{});
  }
  if(view==="Us"){
    renderBurbujaSummary();
    renderSpecialDates();
    scheduleBackground(()=>refreshMemoryCard(false),700);
  }
...
function resetBubbleRuntimeState() {
  ...
  state.savedMessages={};state.sharedNote=null;state.editingNoteId="";state.specialDates=[];state.chatAppearance=null;state.archiveMessages=[];state.archiveCursor=null;state.archiveDone=false;state.archiveLoadedAt=0;state.memoryMessageId="";state.chatPositionRestored=false;state.keyboardSession=null;state.presenceOverride="";state.streakRepairCount=0;state.streakRepairAt=0;state.sendingMessage=false;state.pendingMessages=new Map();state.actionPending=new Map();state.pendingEdits=new Map();state.pendingMessagePatches=new Map();state.pendingStatuses=new Map();state.pendingActivities=new Map();state.pendingGameWrites=new Map();state.pendingMemberPatch={};state.pendingDailyQuestion=null;state.pendingPinned=null;state.pendingRelationship=null;state.pendingSpecialDates=null;state.pendingSavedMessages=null;state.pendingChatAppearance=null;state.pendingNoteData=null;state.dailyActiveWriteAt=0;state.renderedMessageIds=[];state.renderedMessageClientIds=[];state.renderedMessageFirstUnreadId="";state.renderedMessageSearch="";state.messagesDirty=false;
}
...
async function saveBubbleEdit(){
  ...
  const btn=$("#saveBubbleEdit");if(btn)btn.disabled=true;
  const previousBubble=state.bubbles.find(x=>x.id===id)?{...state.bubbles.find(x=>x.id===id)}:null;
  const previousActive=state.activeBubble?{...state.activeBubble}:null;
  const local=state.bubbles.find(x=>x.id===id);if(local){local.name=name;local.emoji=emoji}
  ...
  renderBubbleChooser();$("#editBubbleModal").classList.add("hidden");toast("Burbuja actualizada ♡",1500);haptic(8);
  try{
    await firebase.updateDoc(firebase.doc(state.db,"bubbles",id),{name,emoji,updatedAt:firebase.serverTimestamp()});
    scheduleBackground(()=>loadMemberships().then(()=>renderBubbleChooser()),1000);
  }catch(e){
    if(local&&previousBubble){Object.assign(local,previousBubble)}
    if(state.activeBubbleId===id&&previousActive){state.activeBubble=previousActive;$("#currentBubbleName").textContent=previousActive.name||"Nuestra Burbuja";$("#currentBubbleEmoji").textContent=previousActive.emoji||"🫧"}
    renderBubbleChooser();toast(niceError(e),4200);
...
function activateBubbleShell(bubbleId,bubbleData={}){
  const prebootSame=
    document.documentElement.classList.contains("burbuja-preboot") &&
    state.activeBubbleId===bubbleId;
  const cachedMessages=prebootSame?[...state.messages]:[];
  const cachedProfiles=prebootSame?new Map(state.profiles):new Map();
  const cachedMe=prebootSame?state.me:null;
  const cachedPartner=prebootSame?state.partner:null;
  ...
  // Chat/membresía primero; extras continúan diferidos como en v15.
  wireRealtime();
  scheduleDayBoundaryCheck();

  void updatePresence(true).catch(()=>{});
  ...
  if(prebootSame&&cachedMessages.length){
    requestAnimationFrame(()=>restoreChatPositionOnce());
  }
}
...
async function bootAccount(user) {
  ...
  // Datos locales: permiten pintar la app sin esperar la red.
  ...
  try{
    const cached=JSON.parse(localStorage.getItem(`burbuja-memberships-${user.uid}`)||"[]");
    if(Array.isArray(cached))state.bubbles=cached;
  }catch{}

  const saved=localStorage.getItem(`burbuja-active-${user.uid}`);
  const savedMeta=getCachedActiveBubbleMeta(user.uid);

  // CAMINO RÁPIDO: en una sesión conocida se abre la interfaz de inmediato.
  if(saved&&savedMeta?.id===saved){
    activateBubbleShell(saved,savedMeta);

    // Todo esto ocurre después de que el usuario ya está dentro.
    void validateFastSession(user,saved).catch(()=>{});
    void loadMemberships().catch(e=>console.warn("Refresco de membresías",e));
    return;
  }

  // Primera entrada / caché inexistente: validar normalmente.
  ...
  await loadMemberships();
  if(!state.bubbles.length){
    try{await ensureLegacyMembership()}catch(e){console.warn("No se pudo preparar la migración anterior",e)}
    await loadMemberships();
  }
  if(!state.bubbles.length){
    state.activeBubbleId="";
...
async function loadFirebase() {
  ...
  // Mantener la sesión en este navegador entre aperturas.
  if(firebase.setPersistence && firebase.browserLocalPersistence){
    try{ await firebase.setPersistence(state.auth,firebase.browserLocalPersistence); }catch(e){
      console.warn("No se pudo fijar la persistencia local de Firebase",e);
    }
  }

  showScreen("loading");

  // Chrome móvil: conservar snapshots en IndexedDB para que mensajes,
  // miembros y configuración aparezcan desde caché en aperturas posteriores.
  try{
    if(firebase.initializeFirestore&&firebase.persistentLocalCache){
      const cacheOptions=firebase.persistentMultipleTabManager
        ? {tabManager:firebase.persistentMultipleTabManager()}
        : {};
      state.db=firebase.initializeFirestore(app,{
        localCache:firebase.persistentLocalCache(cacheOptions)
      });
    }else{
      state.db=firebase.getFirestore(app);
    }
  }catch(e){
    console.warn("Caché persistente no disponible; usando caché normal",e);
    state.db=firebase.getFirestore(app);
  }
  ...
  firebase.onAuthStateChanged(state.auth, async (user) => {
    ...
    // A deliberate transition to the login screen must never be reopened by
    // a stale Firebase callback for the previous account.
    if (state.authMode === "forcing-login") {
      clearVisualSession();
      showScreen("auth");
      switchAuthTab("login");
      updateAuthActionButtons();
      if(user){
        try{ await firebase.signOut(state.auth); }catch{}
      }
      if(!user) state.authMode="idle";
      return;
    }
...
function wireRealtime() {
  ...
  state.unsubs.push(firebase.onSnapshot(bubbleCollection("members"), snap => {
    ...
    const remoteMe=state.profiles.get(uid)||state.me||null;
    const pendingMe=state.pendingMemberPatch||{};
    const hasPending=Object.keys(pendingMe).length>0;
    state.me=hasPending?{...remoteMe,...pendingMe}:remoteMe;
    if(state.pendingDailyQuestion){
      const p=state.pendingDailyQuestion;
      state.me={...(state.me||{}),dailyQuestionDate:p.date,dailyQuestionAnswer:p.answer};
      if(remoteMe?.dailyQuestionDate===p.date&&remoteMe?.dailyQuestionAnswer===p.answer)state.pendingDailyQuestion=null;
    }
    if(state.me)state.profiles.set(uid,state.me);
    state.partner=[...state.profiles.entries()].find(([id]) => id !== uid)?.[1] || null;
    if(hasPending&&Object.entries(pendingMe).every(([k,v])=>remoteMe?.[k]===v))state.pendingMemberPatch={};
    cacheMembers();
    ...
    if(state.partner){
      scheduleBackground(()=>repairStreakFromHistory(false),2600);
    }
  }));
  ...
  state.unsubs.push(firebase.onSnapshot(msgQ, snap => {
    ...
    const live=snap.docs.map(d=>{
      ...
      const edit=state.pendingEdits.get(m.id);
      const patch=state.pendingMessagePatches.get(m.id);
      let merged=m;
      if(edit)merged={...merged,...edit};
      if(patch){
        merged={...merged,...patch};
        if(patch.reactions)merged.reactions={...(merged.reactions||{}),...patch.reactions};
      }
      return merged;
    });
    const liveIds=new Set(live.map(m=>m.id));
    const liveClientIds=new Set(live.map(m=>m.clientId).filter(Boolean));

    for(const [clientId,pending] of state.pendingMessages){
      if(liveClientIds.has(clientId)){
        state.pendingMessages.delete(clientId);
      }else if(pending.createdAtMs&&Date.now()-pending.createdAtMs>20000){
        state.pendingMessages.delete(clientId);
      }
    }

    const oldestLive=tsMillis(live[0]?.createdAt);
    const older=state.messages.filter(m=>
      !liveIds.has(m.id)&&
      (!m._pending||state.pendingMessages.has(m.clientId))&&
      (!oldestLive||tsMillis(m.createdAt)<oldestLive)
    );

    const stillPending=state.messages.filter(m=>
      m._pending&&state.pendingMessages.has(m.clientId)
    );

    state.messages=[...older,...live,...stillPending.filter(m=>!liveIds.has(m.id))]
    ...
    const renderMessageJob=()=>{
      if(state.currentView!=="Chat"){state.messagesDirty=true;return;}
      renderMessages();
      requestAnimationFrame(()=>{
        if(firstLoad){
          restoreChatPositionOnce();
          if(state.currentView==="Chat")void markRead().catch(()=>{});
        }else if(wasBottom){
    ...
    if(firstLoad)renderMessageJob();
    else scheduleUI("messages",renderMessageJob);

    state.newestMessageId=newest?.id||"";
    scheduleBackground(()=>cacheRecentMessages(),900);
    scheduleBackground(()=>buildWhileAway(),900);
  }));
  ...
  state.unsubs.push(firebase.onSnapshot(bubbleDoc("shared", "reads"), snap => {
    ...
    scheduleUI("reads",updateMessageReadReceipts);
  }));

  state.unsubs.push(firebase.onSnapshot(bubbleDoc("shared", "pinned"), snap => {
    const remote=snap.exists()?snap.data():{};
    const pending=state.pendingPinned;
    if(pending){
      state.pinned=pending.text||"";
      state.pinnedMessageId=pending.messageId||"";
      if(remote.text===state.pinned&&String(remote.messageId||"")===String(state.pinnedMessageId||""))state.pendingPinned=null;
    }else{
      state.pinned=remote.text||"";
      state.pinnedMessageId=remote.messageId||"";
    }
    scheduleUI("pinned",renderPinned);
  }));

  state.unsubs.push(firebase.onSnapshot(bubbleDoc("shared", "relationship"), snap => {
    const remote=snap.exists()?snap.data():null;
    if(state.pendingRelationship){
      state.relationship={...(remote||{}),...state.pendingRelationship};
      if(remote?.startDate===state.pendingRelationship.startDate)state.pendingRelationship=null;
    }else state.relationship=remote;
    scheduleUI("relationship",renderRelationship);
  }));

  state.unsubs.push(firebase.onSnapshot(bubbleCollection("presence"), snap => {
    state.presence.clear();
    snap.forEach(d => state.presence.set(d.id, d.data()));
    scheduleUI("presence",renderPartnerHeader);
  }));

  state.unsubs.push(firebase.onSnapshot(bubbleCollection("typing"), snap => {
    state.typing.clear();
    snap.forEach(d => state.typing.set(d.id, d.data()));
    scheduleUI("typing",renderTyping);
  }));
  ...
  if("requestIdleCallback" in window)requestIdleCallback(loadExtras,{timeout:1400});
  else setTimeout(loadExtras,650);
}

function wireRealtimeExtras() {
  const uid=state.user?.uid;
  if(!uid||!state.activeBubbleId)return;
  ...
  state.unsubs.push(firebase.onSnapshot(statusQ,snap=>{
    ...
    const live=snap.docs.map(d=>({id:d.id,...d.data()})).filter(s=>{const ok=tsMillis(s.expiresAt)>now;if(!ok)expired.push(s);return ok});
    const liveClientIds=new Set(live.map(s=>s.clientId).filter(Boolean));
    for(const [clientId,pending] of state.pendingStatuses){
      if(liveClientIds.has(clientId))state.pendingStatuses.delete(clientId);
      else if(pending.createdAtMs&&Date.now()-pending.createdAtMs>30000)state.pendingStatuses.delete(clientId);
    }
    const pending=[...state.pendingStatuses.values()].map(x=>x.status).filter(Boolean);
    state.statuses=[...live,...pending.filter(p=>!live.some(x=>x.id===p.id))].filter(s=>tsMillis(s.expiresAt)>now);
    scheduleUI("statuses",()=>{renderStoryRail();buildWhileAway()});
  ...
  state.unsubs.push(firebase.onSnapshot(bubbleDoc("shared","savedMessages"),snap=>{
    ...
    if(state.pendingSavedMessages){
      state.savedMessages=state.pendingSavedMessages.items;
      if(JSON.stringify(remote)===JSON.stringify(state.pendingSavedMessages.items))state.pendingSavedMessages=null;
    }else state.savedMessages=remote;
    scheduleUI("savedMessages",()=>{renderSavedCount();if(!$("#savedModal")?.classList.contains("hidden"))renderSavedMessages()});
  }));

  state.unsubs.push(firebase.onSnapshot(bubbleDoc("shared","sharedNote"),snap=>{
    const remote=snap.exists()?snap.data():null;
    state.sharedNote=state.pendingNoteData?{...(remote||{}),...state.pendingNoteData}:remote;
    if(state.pendingNoteData&&remote?.notes&&JSON.stringify(remote.notes)===JSON.stringify(state.pendingNoteData.notes))state.pendingNoteData=null;
    scheduleUI("sharedNote",renderSharedNote);
  }));
  ...
  state.unsubs.push(firebase.onSnapshot(bubbleDoc("shared","specialDates"),snap=>{
    ...
    state.specialDates=state.pendingSpecialDates?state.pendingSpecialDates.items:remote;
    if(state.pendingSpecialDates&&JSON.stringify(remote)===JSON.stringify(state.pendingSpecialDates.items))state.pendingSpecialDates=null;
    scheduleUI("specialDates",renderSpecialDates);
  }));

  state.unsubs.push(firebase.onSnapshot(bubbleDoc("shared","chatAppearance"),snap=>{
    if(snap.exists()){
      const remote=snap.data();
      state.chatAppearance=state.pendingChatAppearance?{...remote,...state.pendingChatAppearance}:remote;
      if(state.pendingChatAppearance&&Object.entries(state.pendingChatAppearance).every(([k,v])=>remote[k]===v))state.pendingChatAppearance=null;
      scheduleUI("chatAppearance",()=>applyChatAppearance(state.chatAppearance));
    }
  }));
  ...
  state.unsubs.push(firebase.onSnapshot(nudgeQ, snap => {
    ...
    scheduleUI("nudges",renderNudges);
  ...
  state.unsubs.push(firebase.onSnapshot(bubbleDoc("shared", "burbujacraftPoints"), snap => {
    ...
    scheduleUI("burbujacraftPoints",renderBurbujaPoints);
  }));
...
function wireGames() {
  ...
  state.unsubs.push(firebase.onSnapshot(totRef, snap => {
    const remote=snap.exists()?snap.data():null;
    const pending=state.pendingGameWrites.get(`tot:${state.user.uid}`);
    if(pending){
      const remoteVote=remote?.votes?.[state.user.uid];
      if(remoteVote===pending.value)state.pendingGameWrites.delete(`tot:${state.user.uid}`);
      state.totData={...(remote||{}),votes:{...((remote||{}).votes||{}),[state.user.uid]:pending.value}};
    }else state.totData=remote;
    scheduleUI("tot",renderThisOrThat);
  }));

  state.unsubs.push(firebase.onSnapshot(qRef, snap => {
    const remote=snap.exists()?snap.data():null;
    const pending=state.pendingGameWrites.get(`question:${state.user.uid}`);
    if(pending){
      if(remote?.answers?.[state.user.uid]===pending.value)state.pendingGameWrites.delete(`question:${state.user.uid}`);
      state.questionData={...(remote||{}),answers:{...((remote||{}).answers||{}),[state.user.uid]:pending.value}};
    }else state.questionData=remote;
    scheduleUI("dailyQuestion",renderDailyQuestion);
    scheduleBackground(()=>buildWhileAway(),1000);
  }));

  state.unsubs.push(firebase.onSnapshot(compatRef, snap => {
    const remote=snap.exists()?snap.data():null;
    const pending=state.pendingGameWrites.get(`compat:${state.user.uid}`);
    if(pending){
      if(remote?.votes?.[state.user.uid]===pending.value)state.pendingGameWrites.delete(`compat:${state.user.uid}`);
      state.compatData={...(remote||{}),votes:{...((remote||{}).votes||{}),[state.user.uid]:pending.value}};
    }else state.compatData=remote;
    scheduleUI("compat",renderCompatibility);
  }));

  state.unsubs.push(firebase.onSnapshot(guessRef, snap => {
    const remote=snap.exists()?snap.data():null;
    const pending=state.pendingGameWrites.get(`guess:${state.user.uid}`);
    if(pending){
      const answers={...((remote||{}).answers||{}),[state.user.uid]:pending.answer};
      const guesses={...((remote||{}).guesses||{}),[state.user.uid]:pending.guess};
      if(remote?.answers?.[state.user.uid]===pending.answer&&remote?.guesses?.[state.user.uid]===pending.guess)state.pendingGameWrites.delete(`guess:${state.user.uid}`);
      state.guessData={...(remote||{}),answers,guesses};
    }else state.guessData=remote;
    scheduleUI("guess",renderGuessGame);
    scheduleBackground(()=>maybeAwardGuessPoints(),1200);
  }));

  state.unsubs.push(firebase.onSnapshot(streakDayRef, snap => {
    const remote=snap.exists()?snap.data():null;
    const local=state.streakDayData?.active?.[state.user.uid];
    if(local){
      state.streakDayData={...(remote||{}),active:{...((remote||{}).active||{}),[state.user.uid]:local}};
    }else state.streakDayData=remote;
    scheduleUI("streakDay",renderStreak);
  }));

  state.unsubs.push(firebase.onSnapshot(bubbleDoc("shared", "coupleStreak"), snap => {
    state.streakData = snap.exists() ? snap.data() : null;
    scheduleUI("coupleStreak",renderStreak);
  }));

  state.unsubs.push(firebase.onSnapshot(bubbleDoc("shared", "appPoints"), snap => {
    state.appPoints = snap.exists() ? snap.data() : null;
    scheduleUI("appPoints",renderBurbujaPoints);
  }));
}
...
function renderGames(){
  try{renderThisOrThat()}catch{}
  try{renderDailyQuestion()}catch{}
  try{renderCompatibility()}catch{}
  try{renderGuessGame()}catch{}
  try{renderStreak()}catch{}
  try{renderBurbujaPoints()}catch{}
}
...
function renderAll() {
  ...
  renderPinned();
  renderRelationship();
  if(state.currentView==="Chat")renderMessages();else state.messagesDirty=true;
  renderTyping();
  renderThisOrThat();
  renderDailyQuestion();
  renderCompatibility();
...
function renderProfiles() {
  ...
  if(document.activeElement!==$("#profileNameInput"))$("#profileNameInput").value=me.displayName||"";
  if(document.activeElement!==$("#profileHandleInput"))$("#profileHandleInput").value=me.handle||"";
  if(document.activeElement!==$("#profileBioInput"))$("#profileBioInput").value=me.bio||"";
...
function createMessageRow(msg,prev,partnerRead){
  const mine=msg.senderId===state.user.uid;
  const grouped=prev&&prev.senderId===msg.senderId&&tsMillis(msg.createdAt)-tsMillis(prev.createdAt)<180000;
  const row=document.createElement("div");
  row.className=`message-row ${mine?"mine":"theirs"}${grouped?" grouped":""}${msg.systemEvent?" system-event-row":""}${msg._pending?" pending-message-row":""}`;
  row.dataset.messageId=msg.id;
  if(msg.clientId)row.dataset.clientId=msg.clientId;

  const bubble=document.createElement("div");
  bubble.className=`message-bubble${msg.systemEvent?" activity-message":""}`;
  bubble.tabIndex=0;
  ...
  if(msg.replyTo){
    const q=document.createElement("button");q.type="button";q.className="reply-quote";q.innerHTML=`<small></small><span></span>`;
    q.querySelector("small").textContent=msg.replyTo.senderName||"Mensaje";
    q.querySelector("span").textContent=msg.replyTo.text||"📷 Foto / sticker";
    q.onclick=e=>{e.stopPropagation();scrollToMessage(msg.replyTo.id)};
    bubble.appendChild(q);
  }
  if(msg.statusReply){
    const q=document.createElement("div");q.className="status-reply-quote";q.innerHTML=`<small>RESPUESTA A ESTADO</small><strong></strong>`;
    q.querySelector("strong").textContent=msg.statusReply.text||`Estado de ${msg.statusReply.ownerName||"tu persona"}`;
    bubble.appendChild(q);
  }
  if(msg.stickerId||msg.stickerEmoji||msg.stickerAsset){
    const st=stickerById(msg.stickerId)||{emoji:msg.stickerEmoji||"💗",label:msg.stickerText||"",asset:msg.stickerAsset,kind:msg.stickerKind};
    if(st.asset||msg.stickerAsset){
      const im=document.createElement("img");im.className="chat-sticker-image";im.src=msg.stickerAsset||st.asset;im.alt="Sticker";bubble.appendChild(im);
    }else{
      const stBox=document.createElement("div");stBox.className="chat-sticker-text";stBox.innerHTML=`<div class="big"></div><strong></strong>`;
      stBox.querySelector(".big").textContent=st.emoji||msg.stickerEmoji||"💗";
      stBox.querySelector("strong").textContent=st.label||msg.stickerText||"";
      bubble.appendChild(stBox);
    }
  }
  const imgs=(Array.isArray(msg.images)&&msg.images.length?msg.images:(msg.imageData?[msg.imageData]:[]));
  if(imgs.length){
    const grid=document.createElement("div");grid.className=`message-image-grid ${imgs.length===1?"single":""}`;
    imgs.slice(0,4).forEach(src=>{const im=document.createElement("img");im.src=src;im.alt="Foto";im.loading="lazy";im.onclick=e=>{e.stopPropagation();openImage(src)};grid.appendChild(im)});
  ...
  const reactions=msg.reactions||{},groupedReactions={};
  Object.entries(reactions).forEach(([uid,e])=>{if(e)groupedReactions[e]=(groupedReactions[e]||[]).concat(uid)});
  if(Object.keys(groupedReactions).length){
    const r=document.createElement("div");r.className="message-reactions";
    Object.entries(groupedReactions).forEach(([emoji,uids])=>{const b=document.createElement("button");b.type="button";b.className=`reaction-pill ${uids.includes(state.user.uid)?"mine-react":""}`;b.textContent=`${emoji}${uids.length>1?` ${uids.length}`:""}`;b.onclick=e=>{e.stopPropagation();reactToMessage(msg.id,emoji)};r.appendChild(b)});
    bubble.appendChild(r);
  }

  const meta=document.createElement("div");meta.className="message-meta";
  const t=document.createElement("span");t.textContent=timeLabel(msg.createdAt);meta.appendChild(t);
  if(msg.editedAt){const ed=document.createElement("span");ed.className="edited-label";ed.textContent="editado";meta.appendChild(ed)}
  if(mine){
    row.dataset.mine="1";row.dataset.createdAt=String(tsMillis(msg.createdAt)||msg.createdAtMs||0);
    const seen=document.createElement("span"),created=tsMillis(msg.createdAt)||msg.createdAtMs||0,partnerOnline=state.partner&&state.presence.get(state.partner.uid)?.online;
    seen.className="delivery-check";
    seen.textContent=msg._pending?"…":(created&&partnerRead>=created?"✓✓":partnerOnline?"✓✓":"✓");
    seen.title=msg._pending?"Enviando…":(created&&partnerRead>=created?"Visto":partnerOnline?"Entregado":"Enviado");
    meta.appendChild(seen);
  }
  bubble.appendChild(meta);

  let hold=null,sx=0,sy=0;
  bubble.addEventListener("pointerdown",e=>{sx=e.clientX;sy=e.clientY;hold=setTimeout(()=>openMessageActions(msg),520)});
  bubble.addEventListener("pointermove",e=>{if(Math.hypot(e.clientX-sx,e.clientY-sy)>12){clearTimeout(hold);hold=null}});
  bubble.addEventListener("pointerup",e=>{clearTimeout(hold);if(e.clientX-sx>55&&Math.abs(e.clientY-sy)<50){setReply(msg);haptic(10)}});
  bubble.addEventListener("pointercancel",()=>clearTimeout(hold));
  bubble.addEventListener("contextmenu",e=>{e.preventDefault();openMessageActions(msg)});
  bubble.addEventListener("dblclick",()=>openMessageActions(msg));
...
function renderMessages(){
  ...
  const ids=shown.map(m=>m.id),clients=shown.map(m=>m.clientId||"");
  const previousIds=state.renderedMessageIds||[];
  const previousClients=state.renderedMessageClientIds||[];
  const sameSearch=state.renderedMessageSearch===term;
  const sameUnread=state.renderedMessageFirstUnreadId===state.firstUnreadId;
  const canAppend=!term&&sameSearch&&sameUnread&&ids.length===previousIds.length+1&&previousIds.every((id,i)=>ids[i]===id);
  const canReplacePending=!term&&sameSearch&&sameUnread&&ids.length===previousIds.length&&ids.every((id,i)=>id===previousIds[i]||clients[i]&&clients[i]===previousClients[i]);

  if(!shown.length){
    box.innerHTML=`<div class="empty-state"><div class="empty-bubble">🫧</div><h2>${term?"Sin resultados":"Aquí comienza su Burbuja"}</h2><p>${term?"Prueba con otra palabra.":"El primer mensaje siempre cuenta."}</p></div>`;
    state.renderedMessageIds=[];state.renderedMessageClientIds=[];state.renderedMessageSearch=term;state.renderedMessageFirstUnreadId=state.firstUnreadId||"";
    return;
  }

  const partnerRead=state.partner?tsMillis(state.reads[state.partner.uid]):0;

  // Common path: a new message was appended. Keep the existing DOM and only add the new row.
  if(canAppend){
    const msg=shown[shown.length-1],prev=shown[shown.length-2]||null;
    const day=messageDateKey(msg.createdAt);
    const prevDay=prev?messageDateKey(prev.createdAt):"";
    if(day!==prevDay){const sep=document.createElement("div");sep.className="date-separator";sep.innerHTML=`<span>${dateSeparatorLabel(msg.createdAt)}</span>`;box.appendChild(sep)}
    if(state.firstUnreadId&&msg.id===state.firstUnreadId){const nu=document.createElement("div");nu.className="unread-separator";nu.innerHTML="<span>NUEVOS</span>";box.appendChild(nu)}
    box.appendChild(createMessageRow(msg,prev,partnerRead));
    state.renderedMessageIds=ids.slice();state.renderedMessageClientIds=clients.slice();state.renderedMessageSearch=term;state.renderedMessageFirstUnreadId=state.firstUnreadId||"";
    return;
  }

  // Common path when Firestore replaces an optimistic pending message with its real document.
  if(canReplacePending){
    let replaced=false;
    for(let i=0;i<ids.length;i++){
      if(ids[i]===previousIds[i])continue;
      const row=box.querySelector(`[data-client-id="${CSS.escape(clients[i])}"]`);
      if(!row)break;
      const prev=shown[i-1]||null;
      row.replaceWith(createMessageRow(shown[i],prev,partnerRead));
      replaced=true;
    }
    if(replaced){
      state.renderedMessageIds=ids.slice();state.renderedMessageClientIds=clients.slice();state.renderedMessageSearch=term;state.renderedMessageFirstUnreadId=state.firstUnreadId||"";
  ...
  // Full render is reserved for search/filter changes, deletions, older-message loads, or structural changes.
  box.textContent="";
  let prev=null,prevDay="";
  shown.forEach(msg=>{
    const day=messageDateKey(msg.createdAt);
    if(day!==prevDay){const sep=document.createElement("div");sep.className="date-separator";sep.innerHTML=`<span>${dateSeparatorLabel(msg.createdAt)}</span>`;box.appendChild(sep);prevDay=day;prev=null}
    if(state.firstUnreadId&&msg.id===state.firstUnreadId){const nu=document.createElement("div");nu.className="unread-separator";nu.innerHTML="<span>NUEVOS</span>";box.appendChild(nu);prev=null}
    box.appendChild(createMessageRow(msg,prev,partnerRead));
    prev=msg;
  });
  state.renderedMessageIds=ids.slice();state.renderedMessageClientIds=clients.slice();state.renderedMessageSearch=term;state.renderedMessageFirstUnreadId=state.firstUnreadId||"";
...
async function sendActivityMessage({key,icon="✨",title,subtitle="",action="",statusId=""}){
  if(!state.user||!state.activeBubbleId)return false;
  const pendingKey=`activity:${key||title||Date.now()}`;
  if(state.pendingActivities.has(pendingKey))return false;
  ...
  state.pendingActivities.set(pendingKey,{clientId,createdAtMs,statusId:key});
  state.pendingMessages.set(clientId,{createdAtMs,kind:"activity",pendingKey});
  state.messages=[...state.messages,optimistic].sort((a,b)=>tsMillis(a.createdAt)-tsMillis(b.createdAt));
  if(state.currentView==="Chat"){renderMessages();requestAnimationFrame(()=>scrollMessages())}else state.messagesDirty=true;

  try{
    if(key && !(await claimActivityNotice(key))){
      state.pendingActivities.delete(pendingKey);
      state.pendingMessages.delete(clientId);
      state.messages=state.messages.filter(m=>m.clientId!==clientId);
      if(state.currentView==="Chat")renderMessages();else state.messagesDirty=true;
      return false;
    }
    await firebase.addDoc(bubbleCollection("messages"),{
      senderId:state.user.uid,text:"",images:[],clientId,
      systemEvent:{icon,title,subtitle,action,statusId},
      createdAt:firebase.serverTimestamp()
    });
    return true;
    ...
    state.messages=state.messages.filter(m=>m.clientId!==clientId);
    if(state.currentView==="Chat")renderMessages();else state.messagesDirty=true;
    console.warn("activity message",e);
    return false;
  }finally{
    // Keep the pending entry until the Firestore snapshot contains clientId, but never longer than 30s.
    setTimeout(()=>{state.pendingActivities.delete(pendingKey);state.pendingMessages.delete(clientId)},30000);
  }
}
async function upsertStoryActivityMessage(){
  ...
  const previousExisting=existing?{...existing,systemEvent:existing.systemEvent?{...existing.systemEvent}:existing.systemEvent}:null;
  const count=Number(existing?.systemEvent?.count||0)+1;
  const optimistic={
    id,clientId:`status_activity_${id}`,createdAt:existing?.createdAt||Date.now(),createdAtMs:existing?.createdAtMs||Date.now(),
    senderId:state.user.uid,text:"",images:[],systemEvent:{icon:"📸",title:count===1?"Actualizó su estado":`Añadió ${count} estados nuevos`,subtitle:"Toca para ver sus historias",action:"stories",count}
  };
  if(existing)Object.assign(existing,optimistic);else state.messages=[...state.messages,optimistic].sort((a,b)=>tsMillis(a.createdAt)-tsMillis(b.createdAt));
  if(state.currentView==="Chat")renderMessages();else state.messagesDirty=true;
  try{
    const ref=bubbleDoc("messages",id),snap=await firebase.getDoc(ref),prev=snap.exists()?Number(snap.data()?.systemEvent?.count||1):0,finalCount=Math.max(prev+1,count);
    await firebase.setDoc(ref,{senderId:state.user.uid,text:"",images:[],systemEvent:{icon:"📸",title:finalCount===1?"Actualizó su estado":`Añadió ${finalCount} estados nuevos`,subtitle:"Toca para ver sus historias",action:"stories",count:finalCount},createdAt:snap.exists()?(snap.data().createdAt||firebase.serverTimestamp()):firebase.serverTimestamp(),lastActivityAt:firebase.serverTimestamp()},{merge:true});
  }catch(e){
    if(existing&&previousExisting)Object.assign(existing,previousExisting);else state.messages=state.messages.filter(m=>m.id!==id);
    if(state.currentView==="Chat")renderMessages();else state.messagesDirty=true;console.warn("story activity",e);
  }
}
function handleActivityMessageAction(ev){
  ...
  if(ev.action==="stories"){
    switchView("Us");
    requestAnimationFrame(()=>{if(state.partner?.uid)openStoryViewer(state.partner.uid,0)});
  }else if(ev.action==="questions"){
    switchView("Games");
    requestAnimationFrame(()=>$("#dailyAnswer")?.focus({preventScroll:true}));
...
function refreshTodayKeyIfNeeded(){
  ...
  scheduleBackground(()=>repairStreakFromHistory(true),1200);
  return true;
}
...
function renderWhileAway(){
  ...
  state.whileAwayItems.forEach(item=>{
    ...
    b.onclick=()=>{
      if(item.action==="chat"){switchView("Chat");requestAnimationFrame(()=>scrollMessages({smooth:true}))}
...
function renderDailyQuestion() {
  ...
  const mine = state.me?.dailyQuestionDate===state.todayKey
    ? (state.me.dailyQuestionAnswer || "")
    : (answers[state.user.uid] || "");
  const partner = state.partner
    ? (state.partner.dailyQuestionDate===state.todayKey ? (state.partner.dailyQuestionAnswer || "") : (answers[state.partner.uid] || ""))
    : "";
...
async function markDailyActive(source="app"){
  ...
  refreshTodayKeyIfNeeded();
  const uid=state.user.uid,day=state.todayKey;
  const dayRef=bubbleDoc("games",`streak_${day}`);
  const now=Date.now();

  // The visual part of the streak is local and immediate. Firebase is synchronization only.
  const previous=state.streakDayData?{...state.streakDayData,active:{...(state.streakDayData.active||{})}}:null;
  const active={...((state.streakDayData?.active)||{}),[uid]:{source,at:now}};
  const partnerUid=state.partner?.uid;
  const complete=!!partnerUid&&!!active[partnerUid];
  state.streakDayData={...(state.streakDayData||{}),type:"streakDay",date:day,active,...(complete?{completed:true}:{})};
  scheduleUI("streakDay",renderStreak);

  // Do not start a transaction/history repair for every click/message.
  if(now-state.dailyActiveWriteAt<2500)return;
  state.dailyActiveWriteAt=now;

  scheduleBackground(async()=>{
    try{
      await firebase.setDoc(dayRef,{type:"streakDay",date:day,[`active.${uid}`]:{source,at:now},updatedAt:firebase.serverTimestamp()},{merge:true});
      if(!partnerUid)return;

      const newlyCompleted=await firebase.runTransaction(state.db,async tx=>{
        const snap=await tx.get(dayRef),data=snap.exists()?snap.data():{},remoteActive=data.active||{};
        if(!remoteActive[uid]||!remoteActive[partnerUid]||data.completed)return false;
      ...
      scheduleBackground(async()=>{
        ...
        if(newlyCompleted){
          await Promise.all([
            awardAppPoints(uid,`streak_${day}`,2,"Racha diaria"),
            awardAppPoints(partnerUid,`streak_${day}`,2,"Racha diaria")
          ]);
          toast(`🔥 ¡Racha de ${count} ${count===1?"día":"días"}! +2 puntos cada uno`,3600);
          void sendActivityMessage({key:`streak_complete_${day}`,icon:"🔥",title:`¡La racha llegó a ${count} ${count===1?"día":"días"}!`,subtitle:"Los dos estuvieron presentes hoy ♡",action:"streak"});
        }
      },2200);
    }catch(e){
      // Keep the local UI responsive; if the write really fails, restore the previous visual state.
      if(previous){state.streakDayData=previous;scheduleUI("streakDay",renderStreak)}
...
async function voteCompatibility(choice){
  ...
  state.compatData={
    ...
    votes:{...previous.votes,[uid]:choice}
  };
  state.pendingGameWrites.set(`compat:${uid}`,{value:choice});
  renderCompatibility();
  haptic(8);
  ...
  try{
    ...
    state.pendingGameWrites.delete(`compat:${uid}`);
    state.compatData=previous;
    renderCompatibility();
    toast(niceError(e));
    return;
  }

  void markDailyActive("compatibility").catch(()=>{});
  void sendActivityMessage({
    key:`questions_${state.todayKey}_${uid}`,
    icon:"💭",title:"Respondió preguntas de pareja",
    subtitle:"Toca para responder las tuyas",action:"questions"
  }).catch(()=>{});
}
...
async function saveGuessGame(){
  ...
  const previous={
    ...(state.guessData||{}),
    answers:{...((state.guessData||{}).answers||{})},
    guesses:{...((state.guessData||{}).guesses||{})}
  };
  ...
  state.guessData={
    ...
    question:game.q,
    options:game.options,
    answers:{...previous.answers,[uid]:state.guessMineChoice},
    guesses:{...previous.guesses,[uid]:state.guessPartnerChoice}
  };
  state.pendingGameWrites.set(`guess:${uid}`,{answer:state.guessMineChoice,guess:state.guessPartnerChoice});
  renderGuessGame();
  ...
  try{
    await firebase.setDoc(ref,{
      ...
      question:game.q,
      options:game.options,
      [`answers.${uid}`]:state.guessMineChoice,
      [`guesses.${uid}`]:state.guessPartnerChoice,
      updatedAt:firebase.serverTimestamp()
    },{merge:true});
  }catch(e){
    state.pendingGameWrites.delete(`guess:${uid}`);
    state.guessData=previous;
    renderGuessGame();
    toast(niceError(e));
    return;
  }

  void markDailyActive("guess").catch(()=>{});
  void sendActivityMessage({
    key:`questions_${state.todayKey}_${uid}`,
    icon:"💭",title:"Respondió preguntas de pareja",
    subtitle:"Toca para responder las tuyas",action:"questions"
...
async function sendSticker(stickerId){
  ...
  const clientId=`stk_${Date.now()}_${Math.random().toString(36).slice(2,8)}`;
  ...
  const optimistic={
    ...
    stickerAsset:sticker.asset||"",
    stickerKind:sticker.kind||"phrase",
    replyTo:reply
  };

  state.pendingMessages.set(clientId,{createdAtMs});
  state.messages=[...state.messages,optimistic];
  const recent=getStickerSaved("recent").filter(x=>x!==sticker.id);
  setStickerSaved("recent",[sticker.id,...recent]);
  clearComposeContext();
  ...
  void markDailyActive("sticker").catch(()=>{});

  void firebase.addDoc(bubbleCollection("messages"),{
    senderId:state.user.uid,
    clientId,
    text:"",
    images:[],
...
async function sendMessage(text, imageData=""){
  ...
  if(state.editMessageId){
    if(!clean)return toast("El mensaje no puede quedar vacío.");
    const msg=state.messages.find(m=>m.id===state.editMessageId);
    if(!msg||msg.senderId!==state.user.uid)return clearComposeContext();
    const previous={text:msg.text||"",editedAt:msg.editedAt||null};
    const nextText=clean.slice(0,3500),editedAtMs=Date.now();
    msg.text=nextText;msg.editedAt=editedAtMs;
    state.pendingEdits.set(msg.id,{text:nextText,editedAt:editedAtMs});
    $("#messageInput").value="";localStorage.removeItem(draftKey());clearComposeContext();autoGrow($("#messageInput"));renderMessages();haptic(10);
    void firebase.updateDoc(bubbleDoc("messages",msg.id),{text:nextText,editedAt:firebase.serverTimestamp()})
      .then(()=>setTimeout(()=>state.pendingEdits.delete(msg.id),5000))
      .catch(e=>{msg.text=previous.text;msg.editedAt=previous.editedAt;state.pendingEdits.delete(msg.id);renderMessages();toast(niceError(e),3800)});
    return;
  }

  const images=state.pendingImages.length?[...state.pendingImages]:(imageData?[imageData]:[]);
  ...
  void setTyping(false).catch(()=>{});
  void markDailyActive(images.length?"photo":"message").catch(()=>{});
  ...
  void firebase.addDoc(bubbleCollection("messages"),payload)
    .catch(err=>{
      state.pendingMessages.delete(clientId);
      state.messages=state.messages.filter(m=>m.clientId!==clientId);
      renderMessages();
      toast(`No se pudo enviar el mensaje: ${niceError(err)}`,4200);
    });
}

async function saveStatus(emoji, text) {
  if(!state.user||!state.me)return;
  const nextText=String(text).trim().slice(0,60);
  const previous={...state.me};
  const patch={statusEmoji:emoji,statusText:nextText};
  state.pendingMemberPatch={...(state.pendingMemberPatch||{}),...patch};
  state.me={...state.me,...patch,updatedAt:Date.now()};
  state.profiles.set(state.user.uid,state.me);
  renderProfiles();
  renderStatusOptions();
  haptic(8);
  toast("Estado actualizado ✨",1800);
  $("#statusModal").classList.add("hidden");
  $("#customStatusInput").value="";

  const token=Date.now();
  state.actionPending.set("status",token);
  try{
    await firebase.updateDoc(bubbleMemberDoc(),{...patch,updatedAt:firebase.serverTimestamp()});
    if(state.actionPending.get("status")===token){
      const remote={...state.me};
      state.accountProfile={...(state.accountProfile||{}),statusEmoji:emoji,statusText:nextText};
      setCachedAccountProfile(state.user.uid,state.accountProfile);

    }
  }catch(e){
    if(state.actionPending.get("status")===token){
      state.me=previous;state.profiles.set(state.user.uid,state.me);state.pendingMemberPatch={};renderProfiles();renderStatusOptions();toast(niceError(e),3800);
    }
  }finally{if(state.actionPending.get("status")===token)state.actionPending.delete("status")}
}
async function sendNudge() {
  if(!state.user||!state.partner)return toast("Falta que se una tu persona 🫧");
  const createdAtMs=Date.now();
  const optimistic={id:`pending_nudge_${createdAtMs}`,senderId:state.user.uid,targetId:state.partner.uid,createdAt:createdAtMs,createdAtMs,_pending:true};
  const previous=[...state.lastNudges];
  state.lastNudges=[optimistic,...state.lastNudges].slice(0,20);
  renderNudges();
  haptic(8);
  toast("Toquecito enviado 💗",1500);
  try{
    await firebase.addDoc(bubbleCollection("nudges"),{senderId:state.user.uid,targetId:state.partner.uid,createdAt:firebase.serverTimestamp()});
  }catch(e){
    state.lastNudges=previous;renderNudges();toast(niceError(e),3800);
  }
}
async function saveProfile() {
  ...
  const previousProfile={...(state.accountProfile||{})};
  const previousMe={...(state.me||{})};
  const nextProfile={...previousProfile,uid,displayName,handle:newHandle,bio};
  const patch={displayName,handle:newHandle,bio};
  const token=Date.now();
  state.actionPending.set("profile",token);

  // Paint first. Network/alias validation happens behind the UI.
  state.accountProfile=nextProfile;
  state.pendingMemberPatch={...(state.pendingMemberPatch||{}),...patch};
  state.me={...(state.me||{}),...patch,uid};
  state.profiles.set(uid,state.me);
  setCachedAccountProfile(uid,state.accountProfile);
  renderProfiles();
  ...
  try{
    ...
    if(newHandle!==oldHandle){
      ...
      if(existing.exists()&&existing.data().uid!==uid)throw new Error("Ese nombre de usuario ya está ocupado.");
      batch.set(newAliasRef,{uid,email:state.user.email,createdAt:firebase.serverTimestamp()});
      if(oldHandle)batch.delete(firebase.doc(state.db,"aliases",oldHandle));
    }
    batch.update(firebase.doc(state.db,"profiles",uid),{displayName,handle:newHandle,bio,updatedAt:firebase.serverTimestamp()});
    await batch.commit();
    if(state.actionPending.get("profile")===token){
      scheduleBackground(()=>syncProfileToAllBubbles(patch),800);
    }
  }catch(e){
    if(state.actionPending.get("profile")===token){
      state.accountProfile=previousProfile;state.me=previousMe;state.profiles.set(uid,state.me);state.pendingMemberPatch={};setCachedAccountProfile(uid,state.accountProfile);renderProfiles();toast(niceError(e),4200);
    }
  }finally{if(state.actionPending.get("profile")===token)state.actionPending.delete("profile")}
}
async function voteThisOrThat(choice){
  ...
  state.pendingGameWrites.set(`tot:${uid}`,{value:choice});
  renderThisOrThat();
  haptic(8);
  ...
  try{
    ...
    state.pendingGameWrites.delete(`tot:${uid}`);
    state.totData=previous;
    renderThisOrThat();
    toast(niceError(e));
    return;
  }

  void markDailyActive("this-or-that").catch(()=>{});
  void sendActivityMessage({
    key:`questions_${state.todayKey}_${uid}`,
    icon:"💭",title:"Respondió preguntas de pareja",
    subtitle:"Toca para responder las tuyas",action:"questions"
...
async function saveDailyAnswer(){
  ...
  const uid=state.user.uid;
  const date=state.todayKey;
  const previousMember=state.me?{...state.me}:null;
  const previousQuestion={
    ...(state.questionData||{}),
    answers:{...((state.questionData||{}).answers||{})}
  };

  // La respuesta de cada persona queda en su propio documento de miembro.
  // Ese documento ya se sincroniza en tiempo real para ambos integrantes.
  state.pendingDailyQuestion={date,answer};
  state.me={...(state.me||{}),dailyQuestionDate:date,dailyQuestionAnswer:answer};
  state.profiles.set(uid,state.me);
  state.questionData={
    ...(state.questionData||{}),
    type:"question",
    ...
    answers:{...previousQuestion.answers,[uid]:answer}
  };
  state.pendingGameWrites.set(`question:${uid}`,{value:answer});
  renderDailyQuestion();
  toast("Respuesta guardada 💗");
  haptic(8);

  try{
    await firebase.setDoc(bubbleMemberDoc(uid),{
      dailyQuestionDate:date,
      dailyQuestionAnswer:answer,
      updatedAt:firebase.serverTimestamp()
    },{merge:true});

    // Conservamos también el documento anterior de games para compatibilidad
    // con datos ya guardados. Un fallo aquí no invalida la respuesta principal.
    try{
      await firebase.setDoc(bubbleDoc("games",`question_${date}`),{
        type:"question",
        date,
        question:getQuestionForToday(),
        [`answers.${uid}`]:answer,
      ...
      console.warn("No se pudo actualizar el respaldo de la pregunta",e);
    }

    if(state.pendingDailyQuestion?.date===date&&state.pendingDailyQuestion?.answer===answer)state.pendingDailyQuestion=null;
  }catch(e){
    state.pendingGameWrites.delete(`question:${uid}`);
    state.pendingDailyQuestion=null;
    state.me=previousMember;
    if(previousMember)state.profiles.set(uid,previousMember);
    state.questionData=previousQuestion;
    renderDailyQuestion();
    toast(niceError(e));
    return;
  }

  void markDailyActive("daily-question").catch(()=>{});
  void sendActivityMessage({
    key:`questions_${date}_${uid}`,
    icon:"💭",title:"Respondió preguntas de pareja",
    subtitle:"Toca para responder las tuyas",action:"questions"
  }).catch(()=>{});
}
...
function ensurePianoBurbujaCard(){
  const games=$(".featured-games");
  if(!games||$("#openPianoBurbuja"))return;

  const card=document.createElement("article");
  card.className="featured-game piano-game";
  card.innerHTML=`
    <div class="game-art piano-art" aria-hidden="true">
      <div class="piano-keys">
        <i></i><i></i><i></i><i></i>
      </div>
      <span class="piano-bubble piano-bubble-a">♪</span>
      <span class="piano-bubble piano-bubble-b">♫</span>
      <span class="piano-pulse"></span>
    </div>
    <div class="featured-copy">
      <span class="game-tag">MUSICAL · 19 CANCIONES</span>
      <h2>PIANO BURBUJA</h2>
      <p>Toca las fichas al ritmo, mejora tu precisión y suma hasta 20 puntos Burbuja por canción.</p>
      <button id="openPianoBurbuja" class="game-launch-btn piano-launch" type="button">Jugar Piano Burbuja</button>
    </div>
  `;

  const hungry=games.querySelector(".hungry-game");
  if(hungry)hungry.insertAdjacentElement("afterend",card);
  else games.appendChild(card);
}

function pianoPointKey(uid,songId){
  return `${uid}_${songId}`.replace(/[^a-zA-Z0-9_-]/g,"_").slice(0,180);
}

async function awardPianoBurbujaPoints(detail){
  if(!state.user||!state.db||!state.activeBubbleId)return 0;
  if(!detail||detail.game!=="PianoBurbuja")return 0;
  if(String(detail.bubbleId||"")!==String(state.activeBubbleId))return 0;

  const songId=String(detail.songId||"").trim().slice(0,100);
  const songTitle=String(detail.songTitle||"Canción").trim().slice(0,120);
  const candidate=Math.max(0,Math.min(20,Math.round(Number(detail.newBest)||0)));
  if(!songId||candidate<=0)return 0;

  const ref=bubbleDoc("shared","appPoints");
  const key=pianoPointKey(state.user.uid,songId);

  try{
    const awarded=await firebase.runTransaction(state.db,async tx=>{
      const snap=await tx.get(ref);
      const data=snap.exists()?snap.data():{};
      const pianoBest={...(data.pianoBest||{})};
      const serverBest=Math.max(0,Math.min(20,Number(pianoBest[key]||0)));
      const delta=Math.max(0,candidate-serverBest);
      if(delta<=0)return 0;

      const byUid={...(data.byUid||{})};
      byUid[state.user.uid]=Number(byUid[state.user.uid]||0)+delta;
      pianoBest[key]=candidate;

      const pianoHistory={...(data.pianoHistory||{})};
      pianoHistory[key]={
        songId,
        songTitle,
        best:candidate,
      ...
      tx.set(ref,{
        ...
        pianoBest,
        pianoHistory,
        total:Number(data.total||0)+delta,
        updatedAt:firebase.serverTimestamp()
      },{merge:true});
    ...
    if(awarded>0){
      toast(`🎹 +${awarded} puntos Burbuja · ${songTitle}`,3200);
      try{await markDailyActive("piano-burbuja")}catch{}
    }
    return awarded;
  }catch(e){
    console.warn("No se pudieron guardar puntos de Piano Burbuja",e);
    toast("La partida se guardó, pero los puntos no pudieron sincronizarse.",3600);
    return 0;
  }
}

function handlePianoBurbujaBridgeEnvelope(data){
  if(!data||data.type!=="burbuja:game-points")return;
  const detail=data.detail;
  if(!detail||detail.game!=="PianoBurbuja")return;
  awardPianoBurbujaPoints(detail);
}

function handleGameBridgeMessage(event){
  if(event.origin!==location.origin)return;
  const frame=$("#gameFrame");
  if(!frame||event.source!==frame.contentWindow)return;
  handlePianoBurbujaBridgeEnvelope(event.data);
}

function handleGameBridgeStorage(event){
  if(event.key!=="burbuja-game-points-bridge"||!event.newValue)return;
  try{handlePianoBurbujaBridgeEnvelope(JSON.parse(event.newValue))}catch{}
}

function consumePendingGameBridge(){
  try{
    const raw=localStorage.getItem("burbuja-game-points-bridge");
    if(!raw)return;
    const data=JSON.parse(raw);
    if(Date.now()-Number(data?.at||0)>1000*60*60*24)return;
    handlePianoBurbujaBridgeEnvelope(data);
  }catch{}
}

const GAME_PATHS = {
  piano: [
    "../PianoBurbuja/index.html",
    "../PianoBurbuja/",
    "../PianoBurbuja/PianoBurbuja.html"
  ],
  burbujacraft: [
    "../burbujacraft/",
    "../Burbujacraft/",
...
function makeGameUrl(path){
  const gameUrl=new URL(path,location.href);
...
function showGameLauncherUrl(gameUrl,title){
  ...
  const modal=$("#gameLauncherModal");
  const frame=$("#gameFrame");
  const titleEl=$("#gameLauncherTitle");
  if(!modal||!frame||!titleEl){
    toast("No pude abrir el visor de juegos.",4200);
    return false;
  }
...
function launchPianoBurbuja(){
  const paths=GAME_PATHS.piano||[];
  if(!paths.length)return toast("No encontré la ruta de PianoBurbuja.",4200);

  toast("Abriendo Piano Burbuja…",1100);
  ...
  const cleanup=()=>{
    ...
    clearTimeout(window.__pianoLaunchTimer);
  };
  ...
  const failAll=()=>{
    ...
    toast("No encontré PianoBurbuja/index.html en GitHub.",5200);
  };
  ...
  const tryPath=()=>{
    ...
    const url=makeGameUrl(paths[attempt++]);
    showGameLauncherUrl(url,"PianoBurbuja");

    clearTimeout(window.__pianoLaunchTimer);
    window.__pianoLaunchTimer=setTimeout(()=>{
      if(finished)return;
      // Si una ruta se queda colgada, probar la siguiente.
      tryPath();
    },7000);
  };
  ...
  const onLoad=()=>{
    ...
    // GitHub Pages carga una página 404 dentro del iframe como un load normal.
    // Como es el mismo origen podemos detectarla y probar la siguiente ruta.
    try{
      const doc=frame.contentDocument;
      const title=String(doc?.title||"").toLowerCase();
      const body=String(doc?.body?.innerText||"").toLowerCase().slice(0,1200);
      const bad=
        title.includes("page not found") ||
        title.includes("404") ||
        body.includes("there isn't a github pages site here") ||
        body.includes("404 file not found");
      ...
      // Una página real cargó. No seguir probando rutas.
      finished=true;
      cleanup();
    }catch{
      // Si no podemos inspeccionarla, el load del iframe ya indica
      // que la navegación arrancó; no bloqueamos al usuario.
      finished=true;
      cleanup();
    }
  };
...
function launchGame(kind, title) {
  const paths = GAME_PATHS[kind] || [];
  if(!paths.length){
    toast(`No encontré la ruta de ${title}.`,4200);
    return;
  }

  const modal=$("#gameLauncherModal"), frame=$("#gameFrame"), titleEl=$("#gameLauncherTitle");
  if(!modal||!frame||!titleEl){
    toast("No pude abrir el visor de juegos.",4200);
    return;
  }

  try{window.__burbujaGameLaunchCleanup?.()}catch{}

  let attempt=0, finished=false, timer=0;
  const cleanup=()=>{
    frame.removeEventListener("load",onLoad);
    clearTimeout(timer);
    if(window.__burbujaGameLaunchCleanup===cleanup)window.__burbujaGameLaunchCleanup=null;
  };
  const finish=()=>{finished=true;cleanup()};
  const failAll=()=>{
    if(finished)return;
    finish();
    frame.src="about:blank";
  ...
  const is404=()=>{
    try{
      ...
      const body=String(doc?.body?.innerText||"").toLowerCase().slice(0,1600);
      return t.includes("page not found")||t.includes("404")||
        body.includes("there isn't a github pages site here")||
        body.includes("404 file not found")||body.includes("file not found");
    }catch{return false}
  };
  const tryPath=()=>{
    if(finished)return;
    if(attempt>=paths.length){failAll();return;}
    const url=makeGameUrl(paths[attempt++]);
    showGameLauncherUrl(url,title);
    clearTimeout(timer);
    timer=setTimeout(()=>{if(!finished)tryPath()},7000);
  };
  function onLoad(){
    if(finished)return;
    if(is404()){
  ...
  window.__burbujaGameLaunchCleanup=cleanup;
  frame.addEventListener("load",onLoad);
  toast(`Abriendo ${title}…`,1000);
  tryPath();
}
...
async function reactToMessage(id,emoji){
  const msg=state.messages.find(m=>m.id===id);if(!msg||!state.user)return;
  const uid=state.user.uid,current=msg.reactions?.[uid]||"",next=current===emoji?"":emoji;
  const previous={...(msg.reactions||{})};
  const reactions={...previous};if(next)reactions[uid]=next;else delete reactions[uid];
  msg.reactions=reactions;
  state.pendingMessagePatches.set(id,{reactions:{[uid]:next||null}});
  renderMessages();haptic(8);
  const fieldValue=next||firebase.deleteField();
  void firebase.updateDoc(bubbleDoc("messages",id),{[`reactions.${uid}`]:fieldValue})
    .then(()=>setTimeout(()=>{const p=state.pendingMessagePatches.get(id);if(p?.reactions?.[uid]===next)state.pendingMessagePatches.delete(id)},5000))
    .catch(e=>{msg.reactions=previous;state.pendingMessagePatches.delete(id);renderMessages();toast(niceError(e))});
}
...
async function saveChatAppearance(patch){
  if(!state.user)return;
  const previous={...(state.chatAppearance||{})};
  const next={theme:state.chatAppearance?.theme||"oscuro",accent:state.chatAppearance?.accent||"#ff4f91",customBackground:state.chatAppearance?.customBackground||"",...patch,updatedBy:state.user.uid};
  state.pendingChatAppearance={theme:next.theme,accent:next.accent,customBackground:next.customBackground};
  applyChatAppearance({...next,updatedAt:Date.now()});
  toast("Apariencia actualizada 🎨",1300);
  try{
    await firebase.setDoc(bubbleDoc("shared","chatAppearance"),{...next,updatedAt:firebase.serverTimestamp()},{merge:true});
  }catch(e){
    state.pendingChatAppearance=null;state.chatAppearance=previous;applyChatAppearance(previous);toast(niceError(e));
  }
}
...
async function toggleSavedMessage(id){
  ...
  const previous={...(state.savedMessages||{})};
  const items={...previous};
  const wasSaved=!!items[id];
  ...
  state.savedMessages=items;state.pendingSavedMessages={items};renderSavedCount();toast(wasSaved?"Quitado de Guardados":"Guardado ⭐",1400);haptic(8);closeMessageActions();
  const ref=bubbleDoc("shared","savedMessages");
  try{
    await firebase.runTransaction(state.db,async tx=>{
      const snap=await tx.get(ref),data=snap.exists()?snap.data():{},remote={...(data.items||{})};
      if(wasSaved)delete remote[id];
      else{
        const keys=Object.keys(remote);
        if(keys.length>=120)delete remote[keys.sort((a,b)=>(remote[a]?.savedAtMs||0)-(remote[b]?.savedAtMs||0))[0]];
        remote[id]={savedBy:state.user.uid,savedAtMs:Date.now()};
      }
      tx.set(ref,{items:remote,updatedAt:firebase.serverTimestamp()},{merge:true});
    });
    setTimeout(()=>{if(state.pendingSavedMessages?.items===items)state.pendingSavedMessages=null},5000);
  }catch(e){state.pendingSavedMessages=null;state.savedMessages=previous;renderSavedCount();toast(niceError(e),3800)}
}
...
function noteProfile(uid){
  if(!uid)return null;
  return state.profiles?.get(uid) ||
    (state.me?.uid===uid?state.me:null) ||
    (state.partner?.uid===uid?state.partner:null) ||
    (state.user?.uid===uid?state.accountProfile:null);
}

function noteAuthorName(uid){
  const p=noteProfile(uid)||{};
  let name=String(p.handle||p.displayName||p.name||"").trim().replace(/^@/,"");
  if(!name)return uid===state.user?.uid?"Ti":"Tu persona";
  return name.charAt(0).toUpperCase()+name.slice(1);
}

function normalizeSharedNotes(data=state.sharedNote){
  if(data&&Array.isArray(data.notes)){
    return data.notes
      ...
      .map(n=>({
        ...
        createdBy:String(n.createdBy||n.updatedBy||""),
        createdAtMs:Number(n.createdAtMs||n.updatedAtMs||tsMillis(n.createdAt)||tsMillis(n.updatedAt)||0),
        updatedBy:String(n.updatedBy||n.createdBy||""),
        updatedAtMs:Number(n.updatedAtMs||tsMillis(n.updatedAt)||n.createdAtMs||0)
      }));
  }

  const legacyText=String(data?.text||"");
  if(legacyText.trim()){
    const owner=String(data?.updatedBy||state.user?.uid||"");
    const when=tsMillis(data?.updatedAt)||Date.now();
    return [{
      id:"legacy_shared_note",
      title:"",
      text:legacyText.slice(0,5000),
      createdBy:owner,
      createdAtMs:when,
      updatedBy:owner,
      updatedAtMs:when
...
function ensureNotesUI(){
  ...
  const sheet=modal.querySelector(".v15-library-sheet");
  const heading=sheet?.querySelector(".sheet-heading");
  if(!sheet||!heading)return false;

  const headingSmall=heading.querySelector("small");
  const headingTitle=heading.querySelector("h2");
  if(headingSmall)headingSmall.textContent="📝 NOTAS DE LA BURBUJA";
  if(headingTitle)headingTitle.textContent="Notas";

  if($("#notesHome")&&$("#noteEditor"))return true;

  ["sharedNoteInput","saveSharedNoteBtn","noteUpdatedLabel"].forEach(id=>$("#"+id)?.remove());

  const home=document.createElement("div");
  home.id="notesHome";
  home.className="notes-home";
  home.innerHTML=`
    <div class="notes-toolbar">
      <div>
        <strong id="notesCountLabel">0 notas</strong>
        <small>Las notas de los dos, en un solo lugar.</small>
      </div>
      <button id="newNoteBtn" class="primary-btn compact" type="button">＋ Nueva nota</button>
    </div>
  ...
  editor.innerHTML=`
    <button id="backToNotesBtn" class="note-back-btn" type="button">‹ Volver a notas</button>
    <div class="note-editor-owner">
      <small id="noteEditorKicker">NUEVA NOTA</small>
      <strong id="noteEditorOwner">Nota</strong>
    </div>
    <label class="note-field">
      <span>Título <small>opcional</small></span>
      <input id="noteTitleInput" maxlength="80" placeholder="Ej. Películas pendientes" />
    </label>
    <label class="note-field note-field-body">
      <span>Nota</span>
      <textarea id="sharedNoteInput" class="shared-note-input" maxlength="5000" rows="10" placeholder="Escribe lo que quieras guardar aquí…"></textarea>
    </label>
    <div class="note-editor-actions">
      <button id="deleteCurrentNoteBtn" class="note-delete-btn hidden" type="button">Eliminar</button>
      <button id="saveSharedNoteBtn" class="primary-btn" type="button">Guardar nota</button>
    </div>
    <p id="noteUpdatedLabel" class="tiny center">Se sincroniza dentro de esta Burbuja.</p>
  `;

  heading.insertAdjacentElement("afterend",home);
  home.insertAdjacentElement("afterend",editor);

  $("#newNoteBtn").addEventListener("click",()=>openNoteEditor(""));
  $("#backToNotesBtn").addEventListener("click",showNotesHome);
...
function noteSortValue(note){
  return Number(note.updatedAtMs||note.createdAtMs||0);
}

function renderSharedNote(){
  const notes=normalizeSharedNotes();
  const mini=$("#noteMini");
  if(mini)mini.textContent=notes.length===1?"1 nota":`${notes.length} notas`;

  if(!ensureNotesUI())return;
  if(!$("#noteEditor").classList.contains("hidden")&&state.editingNoteId)return;

  const box=$("#notesList");
  const count=$("#notesCountLabel");
  if(count)count.textContent=notes.length===1?"1 nota":`${notes.length} notas`;
  if(!box)return;
  box.textContent="";
  ...
  if(!sorted.length){
    box.innerHTML=`
      <div class="notes-empty">
        <span>📝</span>
        <strong>Todavía no tienen notas</strong>
        <small>Crea la primera nota de su Burbuja ♡</small>
      </div>`;
    return;
  }

  sorted.forEach(note=>{
    const author=noteAuthorName(note.createdBy);
    const editedBy=note.updatedBy&&note.updatedBy!==note.createdBy?noteAuthorName(note.updatedBy):"";
    const card=document.createElement("article");
    card.className="note-card";

    card.innerHTML=`
      <button class="note-card-main" type="button">
        <small class="note-owner-label"></small>
        <strong class="note-card-title"></strong>
        <p class="note-card-preview"></p>
        <span class="note-card-meta"></span>
      </button>
      <button class="note-card-edit" type="button" aria-label="Abrir nota">›</button>
    `;

    const preview=(note.text||"").trim().replace(/\s+/g," ");
    card.querySelector(".note-owner-label").textContent=`📝 NOTA DE ${author.toLocaleUpperCase("es-MX")}`;
    card.querySelector(".note-card-title").textContent=note.title.trim()||`Nota de ${author}`;
    card.querySelector(".note-card-preview").textContent=preview||"Nota vacía";
    const when=note.updatedAtMs?relativeTime(note.updatedAtMs):"recién";
    card.querySelector(".note-card-meta").textContent=editedBy?`${when} · editada por ${editedBy}`:when;

    const open=()=>openNoteEditor(note.id);
    card.querySelector(".note-card-main").addEventListener("click",open);
    card.querySelector(".note-card-edit").addEventListener("click",open);
...
function openNoteEditor(id=""){
  ...
  const author=note?noteAuthorName(note.createdBy):noteAuthorName(state.user.uid);
  $("#noteEditorKicker").textContent=note?"EDITANDO NOTA":"NUEVA NOTA";
  $("#noteEditorOwner").textContent=`Nota de ${author}`;
  $("#noteTitleInput").value=note?.title||"";
  $("#sharedNoteInput").value=note?.text||"";

  const canDelete=!!note&&(
    note.createdBy===state.user.uid ||
    (note.id==="legacy_shared_note"&&(!note.createdBy||note.createdBy===state.user.uid))
  );
  $("#deleteCurrentNoteBtn").classList.toggle("hidden",!canDelete);

  const label=$("#noteUpdatedLabel");
  if(label){
    if(note?.updatedAtMs){
      const editedBy=note.updatedBy?noteAuthorName(note.updatedBy):author;
      label.textContent=`Última edición ${relativeTime(note.updatedAtMs)} · ${editedBy}`;
    }else{
      label.textContent=`Nueva nota de ${author}`;
    }
  }
}
...
async function saveSharedNote(){
  if(!state.user)return;
  const title=String($("#noteTitleInput")?.value||"").trim().slice(0,80);
  const text=String($("#sharedNoteInput")?.value||"").slice(0,5000);
  if(!title&&!text.trim())return toast("Escribe algo en la nota.");
  const editingId=state.editingNoteId,now=Date.now();
  const previous=state.sharedNote?JSON.parse(JSON.stringify(state.sharedNote)):null;
  const notes=normalizeSharedNotes().map(n=>({...n}));
  if(editingId){
    const index=notes.findIndex(n=>n.id===editingId);if(index<0)return toast("Esta nota ya no existe.");
    notes[index]={...notes[index],title,text,updatedBy:state.user.uid,updatedAtMs:now};
  }else{
    if(notes.length>=40)return toast("Pueden guardar hasta 40 notas.");
    notes.push({id:`n_${now}_${Math.random().toString(36).slice(2,8)}`,title,text,createdBy:state.user.uid,createdAtMs:now,updatedBy:state.user.uid,updatedAtMs:now});
  }
  const optimistic={...(state.sharedNote||{}),notes,migratedFromSingleNote:true,updatedAtMs:now};
  state.sharedNote=optimistic;state.pendingNoteData=optimistic;renderSharedNote();showNotesHome();toast(editingId?"Nota actualizada 📝":"Nota creada 📝",1500);haptic(8);

  const ref=bubbleDoc("shared","sharedNote");
  try{
    await firebase.runTransaction(state.db,async tx=>{
      const snap=await tx.get(ref),data=snap.exists()?snap.data():{};
      const remoteNotes=normalizeSharedNotes(data);
      if(editingId){
        const index=remoteNotes.findIndex(n=>n.id===editingId);if(index<0)throw new Error("Esta nota ya no existe.");
        remoteNotes[index]={...remoteNotes[index],title,text,updatedBy:state.user.uid,updatedAtMs:now};
      }else{
        if(remoteNotes.length>=40)throw new Error("Pueden guardar hasta 40 notas.");
        remoteNotes.push(notes[notes.length-1]);
      }
      tx.set(ref,{notes:remoteNotes,migratedFromSingleNote:true,updatedAt:firebase.serverTimestamp()},{merge:true});
    });
    setTimeout(()=>{if(state.pendingNoteData===optimistic)state.pendingNoteData=null},5000);
  }catch(e){state.pendingNoteData=null;state.sharedNote=previous;renderSharedNote();toast(niceError(e),3800)}
}
async function deleteCurrentNote(){
  const id=state.editingNoteId;if(!id)return;
  const current=normalizeSharedNotes().find(n=>n.id===id);if(!current)return showNotesHome();
  const canDelete=current.createdBy===state.user.uid||(current.id==="legacy_shared_note"&&(!current.createdBy||current.createdBy===state.user.uid));
  if(!canDelete)return toast("Solo quien creó la nota puede eliminarla.");
  if(!confirm("¿Eliminar esta nota?"))return;
  const previous=state.sharedNote?JSON.parse(JSON.stringify(state.sharedNote)):null;
  const notes=normalizeSharedNotes().filter(n=>n.id!==id);
  const optimistic={...(state.sharedNote||{}),notes,updatedAtMs:Date.now()};
  state.sharedNote=optimistic;state.pendingNoteData=optimistic;state.editingNoteId="";renderSharedNote();showNotesHome();toast("Nota eliminada.",1400);haptic(8);
  try{
    await firebase.runTransaction(state.db,async tx=>{
      const ref=bubbleDoc("shared","sharedNote"),snap=await tx.get(ref),data=snap.exists()?snap.data():{};
      tx.set(ref,{notes:normalizeSharedNotes(data).filter(n=>n.id!==id),updatedAt:firebase.serverTimestamp()},{merge:true});
    });
    setTimeout(()=>{if(state.pendingNoteData===optimistic)state.pendingNoteData=null},5000);
  }catch(e){state.pendingNoteData=null;state.sharedNote=previous;renderSharedNote();toast(niceError(e))}
}
...
async function addSpecialDate(){
  const label=$("#specialDateLabel").value.trim(),date=$("#specialDateValue").value,emoji=$("#specialDateEmoji").value||"❤️";
  if(!label||!date)return toast("Escribe un nombre y elige una fecha.");
  if((state.specialDates||[]).length>=12)return toast("Máximo 12 fechas especiales.");
  const item={id:`d_${Date.now()}_${Math.random().toString(36).slice(2,6)}`,label:label.slice(0,40),date,emoji,createdBy:state.user.uid};
  const previous=[...(state.specialDates||[])];
  const items=[...previous,item];
  state.specialDates=items;state.pendingSpecialDates={items};renderSpecialDates();$("#specialDateLabel").value="";$("#specialDateValue").value="";toast("Fecha guardada 📅",1500);haptic(8);
  const ref=bubbleDoc("shared","specialDates");
  try{
    await firebase.runTransaction(state.db,async tx=>{
      const snap=await tx.get(ref),remote=snap.exists()&&Array.isArray(snap.data().items)?[...snap.data().items]:[];
      if(remote.length>=12)throw new Error("Máximo 12 fechas especiales.");
      if(!remote.some(x=>x.id===item.id))remote.push(item);
      tx.set(ref,{items:remote,updatedAt:firebase.serverTimestamp()},{merge:true});
    });
    setTimeout(()=>{if(state.pendingSpecialDates?.items===items)state.pendingSpecialDates=null},5000);
  }catch(e){state.pendingSpecialDates=null;state.specialDates=previous;renderSpecialDates();toast(niceError(e))}
}
async function removeSpecialDate(id){
  if(!confirm("¿Quitar esta fecha especial?"))return;
  const previous=[...(state.specialDates||[])],items=previous.filter(i=>i.id!==id);
  state.specialDates=items;state.pendingSpecialDates={items};renderSpecialDates();toast("Fecha eliminada",1400);haptic(8);
  try{
    const ref=bubbleDoc("shared","specialDates");
    await firebase.runTransaction(state.db,async tx=>{
      const snap=await tx.get(ref),remote=snap.exists()&&Array.isArray(snap.data().items)?snap.data().items:[];
      tx.set(ref,{items:remote.filter(i=>i.id!==id),updatedAt:firebase.serverTimestamp()},{merge:true});
    });
    setTimeout(()=>{if(state.pendingSpecialDates?.items===items)state.pendingSpecialDates=null},5000);
  }catch(e){state.pendingSpecialDates=null;state.specialDates=previous;renderSpecialDates();toast(niceError(e))}
}
...
async function publishStory(){
  ...
  const btn=$("#publishStoryBtn");if(btn)btn.disabled=true;
  const clientId=`story_${Date.now()}_${Math.random().toString(36).slice(2,8)}`;
  const createdAtMs=Date.now();
  const optimistic={
    id:`pending_${clientId}`,clientId,_pending:true,ownerUid:state.user.uid,text,photoData:photo||"",
    createdAt:createdAtMs,createdAtMs,expiresAt:firebase.Timestamp.fromMillis(createdAtMs+86400000),
    viewedBy:{[state.user.uid]:true},reactions:{}
  };
  state.pendingStatuses.set(clientId,{status:optimistic,createdAtMs});
  state.statuses=[...state.statuses,optimistic];
  state.storyDraftImage="";
  $("#storyCreateModal").classList.add("hidden");
  $("#storyTextInput").value="";
  renderStoryRail();
  toast(`Estado ${mine.length+1}/30 publicado por 24 horas ✨`,1800);
  haptic(12);

  try{
    await firebase.addDoc(bubbleCollection("statuses"),{
      clientId,ownerUid:state.user.uid,text,photoData:photo||"",
      createdAt:firebase.serverTimestamp(),expiresAt:firebase.Timestamp.fromMillis(createdAtMs+86400000),
      viewedBy:{[state.user.uid]:true},reactions:{}
    });
    void upsertStoryActivityMessage().catch(()=>{});
    void markDailyActive("status").catch(()=>{});
  }catch(e){
    state.pendingStatuses.delete(clientId);state.statuses=state.statuses.filter(x=>x.clientId!==clientId);renderStoryRail();toast(niceError(e),4000);
  }finally{if(btn)btn.disabled=false}
}
...
async function reactStory(emoji){
  const s=state.storyViewerItems[state.storyViewerIndex];if(!s||s.ownerUid===state.user.uid)return;
  const uid=state.user.uid,previous={...(s.reactions||{})};
  s.reactions={...previous,[uid]:emoji};
  const live=state.statuses.find(x=>x.id===s.id);if(live)live.reactions={...s.reactions};
  renderStoryViewer();haptic(8);toast(`${emoji} Reacción enviada`,1400);
  void firebase.updateDoc(bubbleDoc("statuses",s.id),{[`reactions.${uid}`]:emoji})
    .catch(e=>{s.reactions=previous;if(live)live.reactions={...previous};renderStoryViewer();toast(niceError(e))});
}
async function replyStory(){
  const s=state.storyViewerItems[state.storyViewerIndex],text=$("#storyReplyInput").value.trim();if(!s||!text)return;
  const owner=s.ownerUid===state.user.uid?state.me:state.partner;
  const clientId=`status_reply_${Date.now()}_${Math.random().toString(36).slice(2,8)}`,createdAtMs=Date.now();
  const optimistic={id:`pending_${clientId}`,clientId,_pending:true,createdAt:createdAtMs,createdAtMs,senderId:state.user.uid,text:text.slice(0,500),images:[],statusReply:{statusId:s.id,ownerName:owner?.displayName||"Estado",text:(s.text||"📷 Foto").slice(0,160)}};
  state.pendingMessages.set(clientId,{createdAtMs});state.messages=[...state.messages,optimistic].sort((a,b)=>tsMillis(a.createdAt)-tsMillis(b.createdAt));
  $("#storyReplyInput").value="";state.presenceOverride="";$("#storyViewerModal").classList.add("hidden");switchView("Chat");renderMessages();requestAnimationFrame(()=>scrollMessages());toast("Respuesta enviada 💗",1400);
  void firebase.addDoc(bubbleCollection("messages"),{clientId,senderId:state.user.uid,text:optimistic.text,images:[],statusReply:optimistic.statusReply,createdAt:firebase.serverTimestamp()})
    .catch(e=>{state.pendingMessages.delete(clientId);state.messages=state.messages.filter(m=>m.clientId!==clientId);renderMessages();toast(niceError(e),3800)});
  void markDailyActive("status_reply").catch(()=>{});
}
async function deleteCurrentStory(){
  ...
  const previousStatuses=[...state.statuses],previousItems=[...state.storyViewerItems],previousIndex=state.storyViewerIndex;
  state.statuses=state.statuses.filter(x=>x.id!==s.id);state.storyViewerItems=state.storyViewerItems.filter(x=>x.id!==s.id);
  ...
  renderStoryRail();toast("Estado eliminado 🗑️",1500);haptic(8);
  try{
    await firebase.deleteDoc(bubbleDoc("statuses",s.id));
  }catch(e){
    state.statuses=previousStatuses;state.storyViewerItems=previousItems;state.storyViewerIndex=previousIndex;renderStoryRail();renderStoryViewer();toast(niceError(e),4000);
  }finally{if(btn)btn.disabled=false}
}
...
function bindUI() {
  ...
  $("#bubbleHubLogout").addEventListener("click",()=>{void handleLogout()});
  ...
  $("#openPianoBurbuja")?.addEventListener("click", () => launchGame("piano", "PianoBurbuja"));
  window.addEventListener("message",handleGameBridgeMessage);
  window.addEventListener("storage",handleGameBridgeStorage);
  queueMicrotask(consumePendingGameBridge);
  ...
  $("#newMessagesBtn").addEventListener("click",()=>{hideNewMessageButton();scrollMessages({smooth:true});void markRead().catch(()=>{})});
  let readScrollTimer=null,positionTimer=null;
  $("#messages").addEventListener("scroll",()=>{
    clearTimeout(positionTimer);positionTimer=setTimeout(saveCurrentChatPosition,180);
    if(nearBottom()){
      hideNewMessageButton();clearTimeout(readScrollTimer);
      if(state.currentView==="Chat")readScrollTimer=setTimeout(()=>void markRead().catch(()=>{}),100);
    }
  },{passive:true});
  ...
  $("#actionPin").addEventListener("click",()=>{
    const m=actionMessage();if(!m)return;
    const text=(m.text||m.stickerText||"📷 Foto").slice(0,140);
    const previous={text:state.pinned||"",messageId:state.pinnedMessageId||""};
    state.pendingPinned={text,messageId:m.id};state.pinned=text;state.pinnedMessageId=m.id;renderPinned();toast("Mensaje fijado 📌",1500);closeMessageActions();
    void firebase.setDoc(bubbleDoc("shared","pinned"),{text,messageId:m.id,updatedBy:state.user.uid,updatedAt:firebase.serverTimestamp()})
      .catch(e=>{state.pendingPinned=null;state.pinned=previous.text;state.pinnedMessageId=previous.messageId;renderPinned();toast(niceError(e),3800)});
  });
  $("#actionDeleteMe").addEventListener("click",()=>{
    const m=actionMessage();if(!m)return;
    const previous=m.hiddenBy||{};m.hiddenBy={...previous,[state.user.uid]:true};state.pendingMessagePatches.set(m.id,{hiddenBy:m.hiddenBy});renderMessages();toast("Oculto para ti",1500);closeMessageActions();
    void firebase.updateDoc(bubbleDoc("messages",m.id),{[`hiddenBy.${state.user.uid}`]:true})
      .then(()=>state.pendingMessagePatches.delete(m.id))
      .catch(e=>{m.hiddenBy=previous;state.pendingMessagePatches.delete(m.id);renderMessages();toast(niceError(e),3800)});
  });
  $("#actionDeleteAll").addEventListener("click",()=>{
    const m=actionMessage();if(!m||m.senderId!==state.user.uid)return;if(!confirm("¿Eliminar este mensaje para ambos?"))return;
    const index=state.messages.findIndex(x=>x.id===m.id),previous=state.messages[index];
    state.messages=state.messages.filter(x=>x.id!==m.id);renderMessages();toast("Mensaje eliminado",1500);closeMessageActions();
    void firebase.deleteDoc(bubbleDoc("messages",m.id))
      .catch(e=>{if(previous){state.messages.splice(Math.max(0,index),0,previous);state.messages.sort((a,b)=>tsMillis(a.createdAt)-tsMillis(b.createdAt));renderMessages()}toast(niceError(e),3800)});
  });
  ...
  $("#openNoteBtn")?.addEventListener("click",()=>{$("#noteModal").classList.remove("hidden");showNotesHome()});
  ...
  $("#savePinnedBtn").addEventListener("click", () => {
    const text=$("#pinnedInput").value.trim().slice(0,140);
    const previous={text:state.pinned||"",messageId:state.pinnedMessageId||""};
    const next={text,messageId:""};
    state.pendingPinned=next;state.pinned=text;state.pinnedMessageId="";renderPinned();haptic(8);toast("Mensaje fijado 📌",1600);
    void firebase.setDoc(bubbleDoc("shared","pinned"),{text,messageId:"",updatedBy:state.user.uid,updatedAt:firebase.serverTimestamp()})
      .catch(e=>{state.pendingPinned=null;state.pinned=previous.text;state.pinnedMessageId=previous.messageId;renderPinned();toast(niceError(e),3800)});
  });

  $("#relationshipDate").addEventListener("change", e => {
    if(!e.target.value)return;
    const previous=state.relationship?{...state.relationship}:null;
    const startDate=e.target.value;
    state.pendingRelationship={startDate};state.relationship={...(state.relationship||{}),startDate};renderRelationship();toast("Fecha guardada ♡",1500);
    void firebase.setDoc(bubbleDoc("shared","relationship"),{startDate,updatedBy:state.user.uid,updatedAt:firebase.serverTimestamp()},{merge:true})
      .catch(err=>{state.pendingRelationship=null;state.relationship=previous;renderRelationship();toast(niceError(err),3800)});
  });
  ...
  $("#avatarInput").addEventListener("change", async e => {
    ...
    toast("Preparando tu foto…",1800);
    try{
      const data=await compressImage(file,360,.68);
      const previous={...(state.accountProfile||{})},previousMe={...(state.me||{})};
      state.accountProfile={...(state.accountProfile||{}),avatarData:data};
      state.pendingMemberPatch={...(state.pendingMemberPatch||{}),avatarData:data};
      state.me={...(state.me||{}),avatarData:data};
      state.profiles.set(state.user.uid,state.me);
      setCachedAccountProfile(state.user.uid,state.accountProfile);
      renderProfiles();
      toast("Foto de perfil actualizada ♡",1600);haptic(10);
      try{
        await firebase.updateDoc(firebase.doc(state.db,"profiles",state.user.uid),{avatarData:data,updatedAt:firebase.serverTimestamp()});
        scheduleBackground(()=>syncProfileToAllBubbles({avatarData:data}),800);
      }catch(err){
        state.accountProfile=previous;state.me=previousMe;state.profiles.set(state.user.uid,state.me);state.pendingMemberPatch={};setCachedAccountProfile(state.user.uid,state.accountProfile);renderProfiles();toast(niceError(err),3800);
      }
  ...
  $("#loginSessionBtn")?.addEventListener("click", showLoginScreen);
...
window.__burbujaDiag = window.__burbujaDiag || {};
window.__burbujaDiag.bindStarted = true;
try{
  bindUI();
  window.__burbujaDiag.bindComplete = true;
  updateAuthActionButtons();

  // Inicio seguro de sesión:
  // no pintar una Burbuja desde caché antes de que Firebase confirme la sesión.
  // Si no existe una sesión válida, onAuthStateChanged abrirá directamente el
  // apartado de Iniciar sesión. Esto evita mostrar la app a un usuario que ya
  // cerró sesión o cuya sesión todavía no ha sido restaurada por Firebase.
  setPrebootMode(false);

  loadFirebase().catch(err => {
    console.error(err);
    window.__burbujaDiag.loadFirebaseError = String(err?.message||err);
    showScreen("setup");
    toast("No se pudo iniciar Firebase. Revisa la configuración.", 5000);
  });
}catch(err){
  console.error(err);
  window.__burbujaDiag.bindError = String(err?.message||err);
  ...
  if(t)t.textContent="Burbuja detectó un error al iniciar.";
}


// v12 evita registrar un Service Worker dentro de /burbuja/.
// La versión anterior podía dejar HTML/CSS/JS de distintas versiones mezclados en iPhone.
async function cleanupLegacyBurbujaCaches(){
  try{
    const parts=location.pathname.split("/").filter(Boolean);
    const isGithubProject=location.hostname.endsWith("github.io")&&parts.length>0;
    const repoRoot=isGithubProject?`/${parts[0]}/`:"/";
    const burbujaScope=`${repoRoot}burbuja/`;
    ...
    if("serviceWorker" in navigator){
      ...
      await Promise.all(regs.filter(r=>{
        try{
          ...
          return p===repoRoot||p===burbujaScope;
        }catch{return false}
      }).map(r=>r.unregister()));
    }
