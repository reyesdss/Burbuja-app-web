...
const MONITO_STICKERS = [
  "abrazo","beso","risa","dormidos","puchero","llorar","enojo","esconder","saludo","corazon","flor","regalo","jugar","comer","celebrar","manos_corazon","cargar","mandar_beso","enamorados","pena","susto","desconfiado","ay_tu","risita","bostezo","telefono","senalar","fiesta"
].map(id=>({id:`monito_${id}`,kind:"monito",asset:`./assets/stickers/monitos/${id}.svg`,label:""}));
...
function allStickerDefs(){return [...STICKERS,...MONITO_STICKERS]}
...
async function loadMemberships() {
  ...
  for (const d of snap.docs) {
    try {
      ...
      if (b.exists() && (b.data().memberIds || []).includes(state.user.uid)) rows.push({id:d.id,...b.data()});
    } catch {}
  }
...
function renderBubbleChooser() {
  ...
  if (!state.bubbles.length) {
    ...
    empty.innerHTML="<strong>Aún no tienes una Burbuja</strong><br><small>Crea una o usa el código que te compartió tu persona.</small>";
  ...
  for (const bubble of state.bubbles) {
    ...
    const small=document.createElement("small");small.textContent=`${Number(bubble.memberCount||bubble.memberIds?.length||1)}/2 personas · Código ${bubble.inviteCode||"—"}`;
    copy.append(strong,small);
    const actions=document.createElement("div");actions.className="bubble-list-actions";
    const enter=document.createElement("button");enter.type="button";enter.className="bubble-enter-btn";enter.textContent=bubble.id===state.activeBubbleId?"Abrir":"Entrar";
    enter.addEventListener("click",()=>enterBubble(bubble.id));
    const code=document.createElement("button");code.type="button";code.className="bubble-copy-btn";code.textContent="Copiar código";
    code.disabled=!bubble.inviteCode;
    code.addEventListener("click",async()=>{try{await navigator.clipboard.writeText(bubble.inviteCode);toast(`Código ${bubble.inviteCode} copiado ♡`)}catch{toast(`Código: ${bubble.inviteCode}`,4200)}});
    actions.append(enter,code);row.append(emoji,copy,actions);list.appendChild(row);
  }
}
...
async function bootAccount(user) {
  ...
  const chosen=state.bubbles.find(b=>b.id===saved)?.id||state.bubbles[0].id;
  await enterBubble(chosen);
}
...
async function registerAccount(name, handleRaw, password) {
  ...
  try {
    ...
    toast("Cuenta creada. Ahora crea o únete a una Burbuja 🫧",3600);
  } catch(err) {
...
function renderPartnerHeader() {
  ...
  if(!p){el.textContent="falta la segunda cuenta";el.classList.remove("game-activity-together");return}
...
function renderStickerTray() {
  ...
  if(state.stickerTab==="phrases")list=STICKERS;else if(state.stickerTab==="monitos")list=MONITO_STICKERS;else if(state.stickerTab==="favorites")list=favs.map(stickerById).filter(Boolean);else list=recent.map(stickerById).filter(Boolean);
...
function toggleStickerTray(force){const tray=$("#stickerTray");if(!tray)return;const shouldOpen=force===undefined?tray.classList.contains("hidden"):!!force;tray.classList.toggle("hidden",!shouldOpen);if(shouldOpen){renderStickerTray();setTyping(false)}}
...
function applyChatTheme(theme){const allowed=["oscuro","estrellas","burbujas","rosa-morado","minimal"];if(!allowed.includes(theme))theme="oscuro";$("#viewChat")?.setAttribute("data-chat-theme",theme);$$("[data-chat-theme]").forEach(b=>b.classList.toggle("active",b.dataset.chatTheme===theme));if(state.user&&state.activeBubbleId)localStorage.setItem(`burbuja-chat-theme-${state.user.uid}-${state.activeBubbleId}`,theme)}
...
function bindUI() {
  ...
  $("#createBubbleForm").addEventListener("submit", async e => {
    ...
    try{await createBubble($("#createBubbleName").value,$("#createBubbleEmoji").value);$("#createBubbleName").value=""}
  ...
  $("#joinBubbleForm").addEventListener("submit", async e => {
    e.preventDefault(); const btn=e.submitter; btn.disabled=true;
    try{await joinBubble($("#joinBubbleCode").value);$("#joinBubbleCode").value=""}
    catch(err){toast(niceError(err),4200)}finally{btn.disabled=false}
  });
  ...
  $$(".sticker-tab").forEach(btn=>btn.addEventListener("click",()=>{state.stickerTab=btn.dataset.stickerTab;$$('.sticker-tab').forEach(b=>b.classList.toggle('active',b===btn));renderStickerTray()}));
  ...
  $("#closeStoryViewer").addEventListener("click",()=>$("#storyViewerModal").classList.add("hidden"));$("#storyPrev").addEventListener("click",()=>stepStory(-1));$("#storyNext").addEventListener("click",()=>stepStory(1));
