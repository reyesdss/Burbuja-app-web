...
const BURBUJA_VERSION = "responsive-actions-fix-20260919";
...
function activateBubbleShell(bubbleId,bubbleData={}){
  ...
  setTimeout(()=>updatePresence(true).catch(()=>{}),80);
...
async function bootAccount(user) {
  ...
  if(saved&&savedMeta?.id===saved){
    ...
    setTimeout(()=>{
      validateFastSession(user,saved).catch(()=>{});
  ...
  if(saved){
    try{
      ...
      setTimeout(()=>loadMemberships().catch(e=>console.warn("Refresco de membresías",e)),150);
      return;
    }catch(e){
...
function renderMessages() {
  ...
  shown.forEach(msg=>{
    ...
    const row=document.createElement("div");row.className=`message-row ${mine?"mine":"theirs"}${grouped?" grouped":""}${msg.systemEvent?" system-event-row":""}${msg._pending?" pending-message-row":""}`;row.dataset.messageId=msg.id;
...
async function sendMessage(text, imageData=""){
  ...
  if(state.editMessageId){
    ...
    try{
      await firebase.updateDoc(bubbleDoc("messages",msg.id),{
        text:clean.slice(0,3500),
        editedAt:firebase.serverTimestamp()
      });
      $("#messageInput").value="";
...
async function saveSharedNote(){
  ...
  try{
    await firebase.runTransaction(state.db,async tx=>{
      const snap=await tx.get(ref);
      const data=snap.exists()?snap.data():{};
      const notes=normalizeSharedNotes(data);

      if(editingId){
        const index=notes.findIndex(n=>n.id===editingId);
        if(index<0)throw new Error("Esta nota ya no existe.");
        notes[index]={
          ...notes[index],
          title,
          text,
        ...
        if(notes.length>=40)throw new Error("Pueden guardar hasta 40 notas.");
        notes.push({
      ...
      tx.set(ref,{
        notes,
        migratedFromSingleNote:true,
        updatedAt:firebase.serverTimestamp()
      },{merge:true});
    });

    toast(editingId?"Nota actualizada 📝":"Nota creada 📝");
    showNotesHome();
  }catch(e){
    toast(niceError(e));
  }
...
async function deleteCurrentNote(){
  ...
  if(!id)return;
  const current=normalizeSharedNotes().find(n=>n.id===id);
  if(!current)return showNotesHome();

  const canDelete=current.createdBy===state.user.uid ||
    (current.id==="legacy_shared_note"&&(!current.createdBy||current.createdBy===state.user.uid));
  ...
  try{
    await firebase.runTransaction(state.db,async tx=>{
      const snap=await tx.get(ref);
      const data=snap.exists()?snap.data():{};
      const notes=normalizeSharedNotes(data).filter(n=>n.id!==id);
      tx.set(ref,{notes,updatedAt:firebase.serverTimestamp()},{merge:true});
    });
    state.editingNoteId="";
...
function bindUI() {
  ...
  setTimeout(consumePendingGameBridge,900);
