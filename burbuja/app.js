...
function updateSessionControls() {
  ...
  const hasSession = hasRealAuthSession();
  loginBtn?.classList.toggle("hidden", hasSession);
  logoutBtn?.classList.toggle("hidden", !hasSession);
}

function clearLocalSessionShell() {
  const uid = state.auth?.currentUser?.uid || state.user?.uid || "";
  setPrebootMode(false);
  if (uid) {
    localStorage.removeItem(`burbuja-active-${uid}`);
    localStorage.removeItem(`burbuja-active-meta-${uid}`);
    if (state.activeBubbleId) localStorage.removeItem(`burbuja-view-${uid}-${state.activeBubbleId}`);
  }
  localStorage.removeItem("burbuja-active-bubble");
  localStorage.removeItem("burbuja-last-auth-uid");
  localStorage.setItem("burbuja-force-login", "1");
  clearSubscriptions();
  state.user = null;
  state.me = null;
  state.partner = null;
...
function showLoginScreen(reason = "") {
  ...
  requestAnimationFrame(() => {
    ...
    if (reason) toast(reason, 2400);
  });
}

function allowFirebaseSession() {
  state.forceLogin = false;
  localStorage.removeItem("burbuja-force-login");
}
...
function realAuthUid() {
  return state.auth?.currentUser?.uid || "";
}
function requireRealAuthUid() {
  const uid = realAuthUid();
  if (!uid) throw new Error("Necesitas iniciar sesión para hacer esto.");
  return uid;
}
function membershipCollection(uid = realAuthUid()) {
  if (!uid) throw new Error("Necesitas iniciar sesión para ver tus Burbujas.");
  return firebase.collection(state.db, "users", uid, "memberships");
}
function membershipDoc(bubbleId, uid = realAuthUid()) {
  if (!uid) throw new Error("Necesitas iniciar sesión para modificar esta Burbuja.");
...
async function showBubbleChooser(fromApp = false) {
  if (!hasRealAuthSession()) {
    showLoginScreen("Inicia sesión para administrar tus Burbujas.");
    return;
  }
...
function prebootFromLocalCache(){
  if (localStorage.getItem("burbuja-force-login") === "1") return false;
...
async function syncProfileToAllBubbles(patch) {
  const uid = requireRealAuthUid();
  const memberships=await firebase.getDocs(membershipCollection(uid));
  for(const d of memberships.docs){
    try{await firebase.setDoc(firebase.doc(state.db,"bubbles",d.id,"members",uid),{...patch,uid,updatedAt:firebase.serverTimestamp()},{merge:true})}catch{}
  }
}

async function bootAccount(user) {
  if (state.forceLogin || localStorage.getItem("burbuja-force-login") === "1") {
...
async function performLogout() {
  state.forceLogin = true;
  localStorage.setItem("burbuja-force-login", "1");
  const authUser = state.auth?.currentUser || null;
  try {
    if (state.activeBubbleId && authUser) await updatePresence(false);
  } catch {}
  clearLocalSessionShell();
  showScreen("auth");
  switchAuthTab("login");
  updateSessionControls();
  try {
    if (state.auth?.currentUser) await firebase.signOut(state.auth);
  } catch (err) {
    console.warn("No se pudo cerrar Firebase; se mantuvo la pantalla de inicio de sesión", err);
  }
}
...
function bindUI() {
  ...
  $("#bubbleHubLogout").addEventListener("click",async()=>{await performLogout()});
