...
function showLoginScreen(){
  Object.keys(localStorage).forEach(k => { if(k.startsWith('burbuja-active') || k.includes('auth')) localStorage.removeItem(k); });
...
async function handleLogout(){
  try {
    ...
    if (firebase && state.auth && state.auth.currentUser) await firebase.signOut(state.auth);
  } catch(e) {
