...
function sessionStillCurrent(generation, uid){
  if(generation!==state.sessionGeneration) return false;
  if(forceLoginRequested()) return false;
  if(!uid || state.user?.uid!==uid) return false;
  if(state.auth?.currentUser?.uid!==uid) return false;
  return true;
}

function updateAuthActionButtons(){
  const loginBtn=$("#loginSessionBtn");
  const logoutBtn=$("#logoutBtn");
  const hasRealSession=!!state.auth?.currentUser;
  if(loginBtn) loginBtn.classList.toggle("hidden",hasRealSession);
  if(logoutBtn) logoutBtn.classList.toggle("hidden",!hasRealSession);
}
...
async function handleLogout(){
  ...
  // Cerrar sesión no puede depender de presencia ni de que Auth ya esté listo.
  try{
    if(firebase && state.auth?.currentUser){
      await firebase.signOut(state.auth);
    }
  }catch(err){
    console.warn("No se pudo cerrar la sesión de Firebase; se mantiene el login",err);
  }
}
...
async function loadMemberships() {
  if(!firebase || !state.db || !state.user || !state.auth?.currentUser) return [];
  const uid=state.user.uid;
  if(!uid || state.auth.currentUser.uid!==uid || forceLoginRequested()) return [];

  const snap = await firebase.getDocs(membershipCollection());
  if(!state.user || state.user.uid!==uid || state.auth?.currentUser?.uid!==uid || forceLoginRequested()) return [];

  const resolved = await Promise.all(snap.docs.map(async d => {
    if(!state.user || state.user.uid!==uid || forceLoginRequested()) return null;
    try {
      const b = await firebase.getDoc(firebase.doc(state.db,"bubbles",d.id));
      if(b.exists() && (b.data().memberIds||[]).includes(uid))
        return {id:d.id,...b.data()};
    } catch {}
    return null;
  }));
  ...
  state.bubbles = rows;
  try{
    localStorage.setItem(`burbuja-memberships-${uid}`,JSON.stringify(
      rows.map(b=>({
        id:b.id,
        name:b.name||"Nuestra Burbuja",
        emoji:b.emoji||"🫧",
        inviteCode:b.inviteCode||"",
        memberCount:b.memberCount||b.memberIds?.length||1,
        memberIds:b.memberIds||[],
        updatedAt:tsMillis(b.updatedAt)
      }))
    ));
  }catch{}
  return rows;
...
async function enterBubble(bubbleId) {
  if(forceLoginRequested() || !state.user || !state.auth?.currentUser) return false;
  const uid=state.user.uid;
  if(!uid || state.auth.currentUser.uid!==uid) return false;

  const bubbleSnap=await firebase.getDoc(firebase.doc(state.db,"bubbles",bubbleId));

  // El usuario puede haber pulsado "Iniciar sesión" mientras Firestore
  // estaba respondiendo. En ese caso esta operación queda invalidada.
  if(forceLoginRequested() || !state.user || state.user.uid!==uid || state.auth?.currentUser?.uid!==uid)
    return false;

  if(!bubbleSnap.exists()||!(bubbleSnap.data().memberIds||[]).includes(uid))
...
async function bootAccount(user) {
  ...
  if(forceLoginRequested() || !state.auth?.currentUser || state.auth.currentUser.uid!==user.uid)
    return false;
  ...
  // CAMINO RÁPIDO: no hay await en este tramo; si el usuario pulsa
  // "Iniciar sesión" después de entrar, showLoginScreen() invalida la
  // generación y limpia la interfaz.
  if(saved&&savedMeta?.id===saved){
    if(!sessionStillCurrent(generation,user.uid)) return false;
    activateBubbleShell(saved,savedMeta);

    // Validaciones secundarias no deben reabrir la interfaz.
  ...
  if(saved){
    if(!sessionStillCurrent(generation,user.uid)) return false;
    try{
      const entered=await enterBubble(saved);
      if(!entered || !sessionStillCurrent(generation,user.uid)) return false;
      void loadMemberships().catch(e=>console.warn("Refresco de membresías",e));
      return true;
    }catch(e){
      if(!sessionStillCurrent(generation,user.uid)) return false;
  ...
  if(!sessionStillCurrent(generation,user.uid)) return false;
  return !!(await enterBubble(state.bubbles[0].id));
}
...
async function loadFirebase() {
  ...
  firebase.onAuthStateChanged(state.auth, async (user) => {
    ...
    try {
      ...
      const generation=state.sessionGeneration;
      const booted=await bootAccount(user);

      // Si durante la carga se solicitó "Iniciar sesión"/"Cerrar sesión",
      // este callback ya no tiene permiso para repintar la aplicación.
      if(generation!==state.sessionGeneration || forceLoginRequested() || !booted){
        updateAuthActionButtons();
        return;
      }
      updateAuthActionButtons();
...
function bindUI() {
  ...
  $("#manageBubblesBtn").addEventListener("click",()=>{
    if(!state.auth?.currentUser){
      showLoginScreen();
      return;
    }
    saveCurrentChatPosition();
    void showBubbleChooser(true);
  });
