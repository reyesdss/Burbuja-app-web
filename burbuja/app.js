...
function setPrebootMode(on){
  ...
  [
    "manageBubblesBtn","logoutBtn","saveProfileBtn","avatarPickerBtn","changePasswordBtn",
...
function bindUI() {
  ...
  $("#logoutBtn").addEventListener("click", async () => {
    // Cerrar sesión debe funcionar incluso cuando la pantalla viene del
    // preboot pero Firebase todavía no tiene un usuario autenticado.
    const user=await waitForAuthenticatedUser();
    try{if (state.activeBubbleId && user) await updatePresence(false);}catch{}

    // Marca la salida para que la caché local no vuelva a abrir la app en
    // el siguiente recargado antes de que Firebase confirme una sesión.
    localStorage.setItem("burbuja-signed-out","1");
    localStorage.removeItem("burbuja-last-auth-uid");

    if (state.auth?.currentUser) {
      try { await firebase.signOut(state.auth); } catch (err) {
        console.warn("No se pudo cerrar la sesión de Firebase", err);
      }
    }
