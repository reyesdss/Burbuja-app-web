...
function realAuthUser(){
  ...
  if(state.user?.uid&&state.user.uid!==authUser.uid)return null;
  return authUser;
}

function setForceLogin(on=true){
  state.forceLogin=!!on;
  if(on){
...
function syncSessionControls(){
  ...
  if(loginBtn){
    const show=state.forceLogin || !realAuthUser();
    loginBtn.classList.toggle("hidden",!show);
  }
}

function clearLocalSessionView(){
  ...
  try{
    const uid=localStorage.getItem("burbuja-last-auth-uid");
    if(uid) localStorage.removeItem(`burbuja-active-${uid}`);
...
async function openLoginScreen(reason=""){
  setForceLogin(true);
  clearLocalSessionView();
  switchAuthTab("login");
  if(reason==="logout")toast("Sesión cerrada. Inicia sesión para continuar.",2200);
  requestAnimationFrame(()=>$("#loginUser")?.focus({preventScroll:true}));
}

async function handleSessionExit(){
  setForceLogin(true);
  try{
    if(state.activeBubbleId && realAuthUser()) await updatePresence(false);
  }catch{}
  clearLocalSessionView();
  try{
    if(firebase && state.auth?.currentUser) await firebase.signOut(state.auth);
  }catch{}
}

function requireRealAuth(){
  if(realAuthUser())return true;
  void openLoginScreen();
  return false;
}

function prebootFromLocalCache(){
  state.forceLogin=localStorage.getItem("burbuja-force-login")==="1";
  if(state.forceLogin){
    syncSessionControls();
    return false;
  }
...
async function loadFirebase() {
  ...
  firebase.onAuthStateChanged(state.auth, async (user) => {
    ...
    state.forceLogin=state.forceLogin || localStorage.getItem("burbuja-force-login")==="1";
    syncSessionControls();
...
function bindUI() {
  ...
  $("#bubbleSwitcher").addEventListener("click",()=>{
    if(!requireRealAuth())return;
    saveCurrentChatPosition();showBubbleChooser(true);
  });
  ...
  $("#loginSessionBtn")?.addEventListener("click", () => {
    void openLoginScreen("login");
  });
