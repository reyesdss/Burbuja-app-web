...
function showLoginScreen(){
  ...
  // Forzar a Firebase a soltar el usuario silenciosamente
...
async function bootAccount(user) {
  const authFlow=state.authFlow;
  const allowAuthenticatedBoot=authFlow==="login"||authFlow==="register"||authFlow==="active";
  if(forceLoginRequested() && !allowAuthenticatedBoot) return;
  setForceLogin(false);
  clearSubscriptions();
  state.user=user;
...
async function finishAuthenticatedUser(user,generation){
  if(!user)return false;
  if(generation!==state.sessionGeneration)return false;
  if(state.authFlow==="forcing-login"||state.authFlow==="logout")return false;
  if(forceLoginRequested() && state.authFlow!=="login" && state.authFlow!=="register" && state.authFlow!=="active")return false;

  setForceLogin(false);

  if(state.authTargetUid && state.authTargetUid!==user.uid)return false;

  if(state.authBootGeneration===generation && state.authBootUid===user.uid){
    state.user=user;
    state.authTargetUid=user.uid;
    state.authFlow="active";
    updateAuthActionButtons();
    return true;
  }

  state.authBootGeneration=generation;
  state.authBootUid=user.uid;
  state.authTargetUid=user.uid;
  state.user=user;
  ...
  if(generation!==state.sessionGeneration)return false;
  if(forceLoginRequested())return false;
  state.authFlow="active";
  updateAuthActionButtons();
  return true;
}
...
async function loadFirebase() {
  ...
  firebase.onAuthStateChanged(state.auth, async (user) => {
    ...
    // Durante el registro, registerAccount controla explícitamente el primer boot.
    if (state.registrationInProgress) return;

    // Un cierre de sesión solicitado desde el perfil siempre debe dejar la pantalla
    // de autenticación disponible, aunque Firebase todavía entregue al usuario anterior.
    if (forceLoginRequested()) {
      state.authFlow="forcing-login";
      setForceLogin(true);
      clearVisualSession();
    ...
    if (!user) {
      // Puede ser el callback tardío del signOut que inició showLoginScreen().
      // No debe borrar un login/registro nuevo que ya está en progreso.
      if(state.authFlow==="login-pending"||state.authFlow==="login"||state.authFlow==="register"){
        showScreen("auth");
        switchAuthTab(state.authFlow==="register"?"register":"login");
        updateAuthActionButtons();
        await updateSlotHint();
        return;
      }
    ...
    if(state.authFlow==="login-pending")return;
    if((state.authFlow==="login"||state.authFlow==="register") && state.authTargetUid && state.authTargetUid!==user.uid)return;

    try {
      const ok=await finishAuthenticatedUser(user,generation);
      if(!ok)return;
    } catch (err) {
...
async function registerAccount(name, handleRaw, password) {
  ...
  try {
    ...
    if(generation!==state.sessionGeneration)return;
    await finishAuthenticatedUser(cred.user,generation);
    if(generation===state.sessionGeneration && state.authFlow==="active")toast("Cuenta creada. Ahora crea o únete a una Burbuja 🫧",3000);
  } catch(err) {
...
async function loginWithHandle(handleRaw, password) {
  const generation=bumpSessionGeneration();
  state.authFlow="login-pending";
  state.authTargetUid="";
  setForceLogin(false);

  const handle = normalizeHandle(handleRaw);
  if (!validHandle(handle)) {
    if(generation===state.sessionGeneration)setForceLogin(true);
  ...
  state.authTargetUid=String(aliasSnap.data().uid||"");
  state.authFlow="login";
  try{
    await firebase.signInWithEmailAndPassword(state.auth, email, password);
    if(generation!==state.sessionGeneration)return;
    // La navegación final la realiza onAuthStateChanged; este callback solo
    // confirma que la autenticación ya fue aceptada por Firebase.
    setForceLogin(false);
  }catch(err){
    if(generation===state.sessionGeneration){
      state.authFlow="login";
