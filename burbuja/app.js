...
async function bootAccount(user) {
  ...
  // Camino rápido: si la cuenta ya tiene una Burbuja, no repetimos la migración antigua.
  await loadMemberships();
  if(!state.bubbles.length){
...
function syncMobileViewport(resetBaseline=false){
  ...
  if(resetBaseline) viewportBaseline=Math.max(window.innerHeight,h);
  else if(h>viewportBaseline) viewportBaseline=h;
  document.documentElement.style.setProperty("--app-height",`${h}px`);
  const keyboardOpen=!!vv && viewportBaseline-h>120;
