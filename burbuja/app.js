...
function wireGames() {
  ...
  // Relectura explícita al entrar: evita que una emisión inicial de caché vacío
  // deje los juegos sin sus respuestas después de cerrar y volver a abrir la app.
  const gameWireBubbleId = state.activeBubbleId;
  const loadSavedGames = async () => {
    if (!state.user || state.activeBubbleId !== gameWireBubbleId || state.todayKey !== date) return;
    const reads = [
      [totRef, "totData", "tot", renderThisOrThat],
      [qRef, "questionData", "dailyQuestion", renderDailyQuestion],
      [compatRef, "compatData", "compat", renderCompatibility],
      [guessRef, "guessData", "guess", renderGuessGame],
      [streakDayRef, "streakDayData", "streakDay", renderStreak]
    ];
    for (const [ref, prop, uiKey, renderer] of reads) {
      try {
        const snap = await firebase.getDoc(ref);
        if (!snap.exists()) continue;
        if (!state.user || state.activeBubbleId !== gameWireBubbleId || state.todayKey !== date) return;
        state[prop] = snap.data();
        scheduleUI(uiKey, renderer);
      } catch (e) {
        console.warn("No se pudo recuperar el juego guardado", uiKey, e);
      }
    }
  };
  void loadSavedGames();
