...
function showLoginScreen() {
  // 1. Prioridad máxima: bloquear Firebase para que no intente reabrir la app
  bumpSessionGeneration();
  setForceLogin(true);

  // 2. Modificar el DOM de manera directa e inmediata
  try {
    const app = document.getElementById("appShell");
    const auth = document.getElementById("authScreen");
    const bubs = document.getElementById("bubbleScreen");
    const load = document.getElementById("loadingScreen");
    
    if (app) app.classList.add("hidden");
    if (bubs) bubs.classList.add("hidden");
    ...
    console.error("Fallo al forzar el DOM:", e);
  }

  // 3. Ejecutar las funciones normales para mantener el estado
  showScreen("auth");
  switchAuthTab("login");

  // 4. Limpieza dentro de un try/catch estricto
  try {
    clearVisualSession();
    updateAuthActionButtons();
  } catch(e) {
    console.warn("Ignorando error en limpieza de sesión:", e);
  }

  // 5. Intentar hacer focus diferido para no bloquear
  setTimeout(() => {
...
function setPrebootMode(on){
  ...
  if(on){
    ...
    if(actions && actions.style) actions.style.pointerEvents="none";
  }else{
    badge?.remove();
    const composer=$("#composer");
    if(composer && composer.style){
    ...
    const actions=$(".header-actions");
    if(actions && actions.style) actions.style.pointerEvents="";
  }
}
