...
async function enterBubble(bubbleId) {
  ...
  const rememberedView=localStorage.getItem(`burbuja-view-${state.user.uid}-${state.activeBubbleId}`)||"Chat";
  ...
  renderAll();
  restoreDraft();
  applyChatAppearance({theme:localStorage.getItem(`burbuja-chat-theme-${state.user.uid}-${state.activeBubbleId}`)||"oscuro",accent:"#ff4f91",customBackground:""});
}
