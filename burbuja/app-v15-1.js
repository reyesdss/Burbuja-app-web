...
function activateBubbleShell(bubbleId,bubbleData={}){
  ...
  const rememberedView=
    localStorage.getItem(`burbuja-view-${state.user.uid}-${bubbleId}`)||
    localStorage.getItem(`burbuja-view-${state.user.uid}-${state.activeBubbleId}`)||
    "Chat";
  ...
  // Presencia y datos secundarios no retrasan la primera pintura.
  setTimeout(()=>updatePresence(true).catch(()=>{}),60);
