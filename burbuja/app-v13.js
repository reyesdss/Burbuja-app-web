...
function wireRealtime() {
  ...
  state.unsubs.push(firebase.onSnapshot(msgQ, snap => {
    ...
    const wasNearBottom=firstLoad || nearBottom();
    const anchor=!wasNearBottom ? captureMessageAnchor() : null;
    ...
    computeUnreadBoundary();
    renderMessages();

    if(wasNearBottom){
      hideNewMessageButton();
      scrollMessages();
      if(state.currentView==="Chat") setTimeout(markRead,80);
    }else{
      requestAnimationFrame(()=>requestAnimationFrame(()=>restoreMessageAnchor(anchor)));
  ...
  state.unsubs.push(firebase.onSnapshot(bubbleDoc("shared", "reads"), snap => {
    ...
    computeUnreadBoundary();
    renderMessagesPreservingPosition();
  }));
...
function renderMessagesPreservingPosition(){
  ...
  const anchor=!wasBottom?captureMessageAnchor():null;
  renderMessages();
  if(wasBottom)scrollMessages();
  else requestAnimationFrame(()=>restoreMessageAnchor(anchor));
}
...
function scheduleDayBoundaryCheck(){
  ...
  window.__burbujaDayCheck=setInterval(()=>refreshTodayKeyIfNeeded(),30000);
}
...
function buildWhileAway(){
  ...
  if(partner){
    ...
    const incoming=state.messages.filter(m=>m.senderId===partner.uid&&tsMillis(m.createdAt)>seenAt&&!m.hiddenBy?.[state.user.uid]);
    if(incoming.length)items.push({icon:"💬",text:`${incoming.length} mensaje${incoming.length===1?"":"s"} nuevo${incoming.length===1?"":"s"}`,action:"chat"});
    if(stories.length)items.push({icon:"📸",text:`${stories.length} estado${stories.length===1?"":"s"} nuevo${stories.length===1?"":"s"}`,action:"stories"});
    const pQuestion=state.questionData?.answers?.[partner.uid];
    if(pQuestion && tsMillis(state.questionData?.updatedAt)>seenAt)items.push({icon:"💭",text:"Respondió la pregunta de hoy",action:"questions"});
  }
  state.whileAwayItems=items;
  renderWhileAway();
}
...
function renderWhileAway(){
  ...
  state.whileAwayItems.forEach(item=>{
    ...
    b.innerHTML=`<span></span><strong></strong>`;b.querySelector("span").textContent=item.icon;b.querySelector("strong").textContent=item.text;
    b.onclick=()=>{
  ...
  try {
    ...
    if (result) {
      ...
      await sendActivityMessage({
        key:`streak_complete_${state.todayKey}`,
        icon:"🔥",
        title:`¡La racha llegó a ${result} ${result===1?"día":"días"}!`,
        subtitle:"Los dos estuvieron presentes hoy ♡",
        action:"streak"
      });
    }
  } catch (e) {
...
async function loadOlderMessages(){
  ...
  const btn=$("#loadOlderBtn"),first=state.messages[0],anchor=captureMessageAnchor();
...
function bindUI() {
  ...
  $("#messageInput").addEventListener("focus", () => {
    ...
    setTimeout(()=>syncMobileViewport(false),30);
    if(keepBottom)setTimeout(()=>scrollMessages(),180);
  });
  ...
  $("#messages").addEventListener("scroll",()=>{
    if(nearBottom()){
      ...
      if(state.currentView==="Chat")readScrollTimer=setTimeout(markRead,350);
    }
  },{passive:true});
