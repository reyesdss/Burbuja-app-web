...
function resetBubbleRuntimeState() {
  ...
  state.statuses = []; state.replyTo=null; state.editMessageId=""; state.messageActionId=""; state.pendingImages=[]; state.searchTerm=""; state.olderExhausted=false; state.storyViewerItems=[]; state.storyViewerIndex=0; state.storyDraftImage=""; state.unreadWhileUp=0; state.newestMessageId="";
}
...
async function enterBubble(bubbleId) {
  ...
  if(!bubbleSnap.exists()||!(bubbleSnap.data().memberIds||[]).includes(state.user.uid))throw new Error("No tienes acceso a esa Burbuja.");
  if(state.activeBubbleId && state.activeBubbleId!==bubbleId){try{await updatePresence(false)}catch{}}
  clearSubscriptions();
  ...
  try{localStorage.setItem(`burbuja-active-meta-${state.user.uid}`,JSON.stringify({id:bubbleId,name:state.activeBubble.name||"Nuestra Burbuja",emoji:state.activeBubble.emoji||"🫧",inviteCode:state.activeBubble.inviteCode||""}))}catch{}
  ...
  $("#appShell")?.setAttribute("data-view", state.currentView || "Chat");
  showScreen("app");
  wireRealtime();
  updatePresence(true);
  ...
  renderAll();
  restoreDraft();
  applyChatTheme(localStorage.getItem(`burbuja-chat-theme-${state.user.uid}-${state.activeBubbleId}`)||"oscuro");
}
...
async function bootAccount(user) {
  ...
  // Carga una lista local únicamente para que los botones/listas aparezcan sin esperar red.
  try{
    const cached=JSON.parse(localStorage.getItem(`burbuja-memberships-${user.uid}`)||"[]");
    if(Array.isArray(cached))state.bubbles=cached;
  }catch{}

  // Camino rápido habitual: un solo getDoc a la Burbuja guardada.
  ...
  if(saved){
    try{
      ...
      return;
    }catch(e){console.warn("La Burbuja guardada ya no está disponible",e)}
  }
  ...
  if(!state.bubbles.length){
    state.activeBubbleId="";state.activeBubble=null;resetBubbleRuntimeState();
    await showBubbleChooser(false);return;
  }
...
function wireRealtime() {
  ...
  state.unsubs.push(firebase.onSnapshot(msgQ, snap => {
    ...
    if(previousNewest&&newest?.id!==previousNewest&&!wasNearBottom&&newest?.senderId!==uid){state.unreadWhileUp++;$("#newMessagesBtn")?.classList.remove("hidden");$("#newMessagesBtn").textContent=`↓ ${state.unreadWhileUp} nuevo${state.unreadWhileUp===1?"":"s"}`}
    renderMessages();
    if (wasNearBottom) requestAnimationFrame(scrollMessages);
    if (state.currentView === "Chat" && wasNearBottom) markRead();
  ...
  if ("requestIdleCallback" in window) requestIdleCallback(loadExtras,{timeout:900});
  else setTimeout(loadExtras,350);
}

function wireRealtimeExtras() {
  const statusQ=firebase.query(bubbleCollection("statuses"),firebase.orderBy("createdAt","desc"),firebase.limit(30));
...
function renderPartnerHeader() {
  ...
  if(online&&activity&&state.currentGameTitle&&activity===state.currentGameTitle){el.textContent=`💜🩷 Jugando juntos · ${activity}`;el.classList.add("game-activity-together")}
  else if(online&&activity)el.textContent=`🎮 Jugando ${activity}`;
...
function renderMessages() {
  ...
  shown.forEach(msg=>{
    ...
    const row=document.createElement("div");row.className=`message-row ${mine?"mine":"theirs"}${grouped?" grouped":""}`;row.dataset.messageId=msg.id;
    const bubble=document.createElement("div");bubble.className="message-bubble";bubble.tabIndex=0;
    ...
    if(msg.text){const txt=document.createElement("div");txt.className="message-text";txt.textContent=msg.text;bubble.appendChild(txt)}
    ...
    const meta=document.createElement("div");meta.className="message-meta";const t=document.createElement("span");t.textContent=timeLabel(msg.createdAt);meta.appendChild(t);if(msg.editedAt){const ed=document.createElement("span");ed.className="edited-label";ed.textContent="editado";meta.appendChild(ed)}if(mine){const seen=document.createElement("span"),created=tsMillis(msg.createdAt),partnerOnline=state.partner&&state.presence.get(state.partner.uid)?.online;seen.textContent=created&&partnerRead>=created?"✓✓":partnerOnline?"✓✓":"✓";seen.title=created&&partnerRead>=created?"Visto":partnerOnline?"Entregado":"Enviado";meta.appendChild(seen)}bubble.appendChild(meta);
...
async function markDailyActive(source = "app") {
  ...
  try {
    ...
    const streakRef = bubbleDoc("shared", "coupleStreak");
...
async function markRead() {
  ...
  try {
    await firebase.setDoc(bubbleDoc("shared", "reads"), {
...
async function updatePresence(online) {
  ...
  try {
    await firebase.setDoc(bubbleDoc("presence", state.user.uid), {
      uid: state.user.uid,
      online,
      bubbleId:state.activeBubbleId,
      activity:state.currentGameTitle || "",
...
function scrollToMessage(id){const row=document.querySelector(`[data-message-id="${CSS.escape(id||"")}"]`);if(!row)return toast("Ese mensaje no está cargado. Prueba cargar anteriores.");row.scrollIntoView({behavior:"smooth",block:"center"});row.animate([{filter:"brightness(1)"},{filter:"brightness(1.8)"},{filter:"brightness(1)"}],{duration:900})}
function openMessageActions(msg){state.messageActionId=msg.id;const mine=msg.senderId===state.user.uid;$("#actionEdit").classList.toggle("hidden",!mine||!msg.text||msg.stickerId);$("#actionDeleteAll").classList.toggle("hidden",!mine);$("#actionCopy").classList.toggle("hidden",!msg.text);$("#messageActionsModal").classList.remove("hidden");haptic(8)}
...
async function loadOlderMessages(){if(state.olderExhausted||!state.messages.length)return;const btn=$("#loadOlderBtn"),first=state.messages[0];if(!first?.createdAt)return;btn.disabled=true;btn.textContent="Cargando…";try{const q=firebase.query(bubbleCollection("messages"),firebase.orderBy("createdAt","asc"),firebase.endBefore(first.createdAt),firebase.limitToLast(80)),snap=await firebase.getDocs(q),older=snap.docs.map(d=>({id:d.id,...d.data()}));const ids=new Set(state.messages.map(m=>m.id));state.messages=[...older.filter(m=>!ids.has(m.id)),...state.messages];state.olderExhausted=older.length<80;btn.classList.toggle("hidden",state.olderExhausted);renderMessages();if(older.length){requestAnimationFrame(()=>scrollToMessage(first.id))}else toast("Ya llegaste al inicio ♡")}catch(e){toast(niceError(e))}finally{btn.disabled=false;btn.textContent="↑ Cargar mensajes anteriores"}}
function applyChatTheme(theme){
  const allowed=["oscuro","estrellas","burbujas","rosa-morado","minimal"];
  if(!allowed.includes(theme))theme="oscuro";
  const view=$("#viewChat"), surface=$("#messages");
  view?.setAttribute("data-chat-theme",theme);
  if(surface){
    const urls={
      "oscuro":"./assets/backgrounds/oscuro.svg?v=12",
      "estrellas":"./assets/backgrounds/estrellas.svg?v=12",
      "burbujas":"./assets/backgrounds/burbujas.svg?v=12",
      "rosa-morado":"./assets/backgrounds/rosa-morado.svg?v=12",
      "minimal":"./assets/backgrounds/minimal.svg?v=12"
    };
    surface.style.backgroundImage=`url("${urls[theme]}")`;
    surface.style.backgroundRepeat="repeat";
    surface.style.backgroundSize=theme==="minimal"?"cover":"360px 260px";
    surface.dataset.theme=theme;
  }
  $$("[data-chat-theme]").forEach(b=>b.classList.toggle("active",b.dataset.chatTheme===theme));
  if(state.user&&state.activeBubbleId)localStorage.setItem(`burbuja-chat-theme-${state.user.uid}-${state.activeBubbleId}`,theme);
}
...
async function publishStory(){const text=$("#storyTextInput").value.trim().slice(0,220),photo=state.storyDraftImage;if(!text&&!photo)return toast("Agrega una foto o escribe algo.");const btn=$("#publishStoryBtn");btn.disabled=true;try{await firebase.addDoc(bubbleCollection("statuses"),{ownerUid:state.user.uid,text,photoData:photo||"",createdAt:firebase.serverTimestamp(),expiresAt:firebase.Timestamp.fromMillis(Date.now()+86400000),viewedBy:{[state.user.uid]:true},reactions:{}});$("#storyCreateModal").classList.add("hidden");state.storyDraftImage="";toast("Estado publicado por 24 horas ✨");haptic(12)}catch(e){toast(niceError(e),4000)}finally{btn.disabled=false}}
function openStoryViewer(uid,index=0){const items=activeStatusesFor(uid);if(!items.length)return;if(uid===state.user.uid&&index===0&&items.length===0)return openStoryCreate();state.storyViewerItems=items;state.storyViewerIndex=Math.max(0,Math.min(index,items.length-1));$("#storyViewerModal").classList.remove("hidden");renderStoryViewer()}
async function renderStoryViewer(){const items=state.storyViewerItems,s=items[state.storyViewerIndex];if(!s)return $("#storyViewerModal").classList.add("hidden");const owner=s.ownerUid===state.user.uid?state.me:state.partner;setAvatar($("#storyViewerAvatar"),owner);$("#storyViewerName").textContent=owner?.displayName||"Estado";$("#storyViewerTime").textContent=relativeTime(s.createdAt);const progress=$("#storyProgress");progress.textContent="";items.forEach((_,i)=>{const p=document.createElement("i");if(i<state.storyViewerIndex)p.className="done";if(i===state.storyViewerIndex)p.className="active";progress.appendChild(p)});const media=$("#storyMedia");media.textContent="";if(s.photoData){const im=document.createElement("img");im.src=s.photoData;im.alt="Estado";media.appendChild(im)}else{const d=document.createElement("div");d.className="story-text-card";d.textContent=s.text||"🫧";media.appendChild(d)}$("#storyCaption").textContent=s.photoData?s.text:"";const seen=s.viewedBy||{};const partnerReaction=s.reactions?.[state.partner?.uid]||"";$("#storySeen").textContent=s.ownerUid===state.user.uid?(seen[state.partner?.uid]?`Visto por ${state.partner?.displayName||"tu persona"}${partnerReaction?` · ${partnerReaction}`:" ♡"}`:"Aún no lo ha visto"):"";$("#deleteStoryBtn")?.classList.toggle("hidden",s.ownerUid!==state.user.uid);if(s.ownerUid!==state.user.uid&&!seen[state.user.uid]){firebase.updateDoc(bubbleDoc("statuses",s.id),{[`viewedBy.${state.user.uid}`]:true}).catch(()=>{})}}
function stepStory(delta){const ni=state.storyViewerIndex+delta;if(ni<0)return;if(ni>=state.storyViewerItems.length)return $("#storyViewerModal").classList.add("hidden");state.storyViewerIndex=ni;renderStoryViewer()}
...
async function deleteCurrentStory(){
  ...
  try{
    ...
    if(!state.storyViewerItems.length){$("#storyViewerModal").classList.add("hidden");}
...
function editableFocused(){
  ...
  return !!a && (a.matches?.('input:not([type="button"]):not([type="submit"]):not([type="checkbox"]):not([type="radio"]), textarea, select') || a.isContentEditable);
}
function syncMobileViewport(resetBaseline=false){
  const vv=window.visualViewport;
  const h=Math.max(300,Math.round(vv?.height || window.innerHeight));
  const scale=vv?.scale||1;
  if(resetBaseline || !editableFocused()) viewportBaseline=Math.max(window.innerHeight,h);
  document.documentElement.style.setProperty("--app-height",`${h}px`);
  const keyboardOpen=editableFocused() && Math.abs(scale-1)<0.02 && (viewportBaseline-h)>105;
  document.documentElement.classList.toggle("keyboard-open",keyboardOpen);
  document.documentElement.classList.toggle("field-focused",editableFocused());
  if(keyboardOpen){
    // iOS sometimes pans the layout viewport while focusing. The app itself must remain at y=0.
...
function bindUI() {
  ...
  $("#bubbleSwitcher").addEventListener("click",()=>showBubbleChooser(true));
  $("#manageBubblesBtn").addEventListener("click",()=>showBubbleChooser(true));
  ...
  $("#messageInput").addEventListener("focus", () => {toggleStickerTray(false);setTimeout(()=>syncMobileViewport(false),30);setTimeout(()=>{try{window.scrollTo(0,0)}catch{};scrollMessages()},160)});
  $("#messageInput").addEventListener("blur", () => setTimeout(()=>syncMobileViewport(true),80));
  ...
  $("#newMessagesBtn").addEventListener("click",()=>{state.unreadWhileUp=0;$("#newMessagesBtn").classList.add("hidden");scrollMessages();markRead()});
  $("#messages").addEventListener("scroll",()=>{if(nearBottom()){state.unreadWhileUp=0;$("#newMessagesBtn").classList.add("hidden");if(state.currentView==='Chat')markRead()}});
  ...
  $$("[data-chat-theme]").forEach(btn=>btn.addEventListener("click",()=>applyChatTheme(btn.dataset.chatTheme)));
  ...
  $("#closeStoryViewer").addEventListener("click",()=>$("#storyViewerModal").classList.add("hidden"));$("#deleteStoryBtn")?.addEventListener("click",deleteCurrentStory);$("#storyPrev").addEventListener("click",()=>stepStory(-1));$("#storyNext").addEventListener("click",()=>stepStory(1));
  ...
  document.addEventListener("focusin",e=>{if(e.target?.matches?.("input,textarea,select"))setTimeout(()=>syncMobileViewport(false),20)});
  document.addEventListener("focusout",e=>{if(e.target?.matches?.("input,textarea,select"))setTimeout(()=>syncMobileViewport(true),100)});
  ...
  window.visualViewport?.addEventListener("scroll",()=>syncMobileViewport(false));
  window.addEventListener("orientationchange",()=>setTimeout(()=>syncMobileViewport(true),180));
  window.addEventListener("resize",()=>{if(!window.visualViewport)syncMobileViewport(true)});
}
...
async function cleanupLegacyBurbujaCaches(){
  try{
    if("serviceWorker" in navigator){
      ...
      await Promise.all(regs.filter(r=>{try{return new URL(r.scope).pathname.includes("/burbuja/")}catch{return false}}).map(r=>r.unregister()));
    }
    if("caches" in window){const keys=await caches.keys();await Promise.all(keys.filter(k=>k.startsWith("burbuja-shell")).map(k=>caches.delete(k)));}
  }catch{}
}
